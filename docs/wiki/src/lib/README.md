...
| `tool-marquee.ts` | subagent 摘要卡「当前工具」跑马灯动画控制：`ToolMarqueeController`（DOM/定时器/动画经参数注入可单测）——溢出且非 reduced-motion 时 WAAPI 循环（35px/s 滚动→端部停顿→回弹），text 变化才重建，常量与侧栏会话标题跑马灯一致 |
