...
describe('session state service facade (storage v2)', () => {
  ...
  it('is always authoritative and reports a constant authoritative state', async () => {
    ...
    expect(await getSessionStateDiagnostics()).toMatchObject({
      phase: 'authoritative',
      authority: 'sqlite',
  ...
  it('preserves opaque body/metadata/message fields and metadata-owned pin/archive on body saves', async () => {
    repository.save(initialRecord(), { expectedRevision: 0 })
    const saved = await saveSessionBody('one', {
      title: 'Updated',
    ...
    expect((await readSessionStateValue('one')).messages).toEqual([expect.objectContaining({ anotherUnknown: true })])
    expect(saved.metadata).toMatchObject({ metadataUnknown: { keep: true }, pinnedAt: '2026-01-01T00:00:00.000Z', messageCount: 1 })
  })

  it('requires an existing body for metadata-only writes and synchronizes body-owned projections', async () => {
    repository.save(initialRecord(), { expectedRevision: 0 })
    await expect(saveSessionMetadata('missing', { pinnedAt: 'x' })).rejects.toMatchObject({ statusCode: 409, errorCode: 'SESSION_STATE_REQUIRED' })
    const saved = await saveSessionMetadata('one', {
  ...
  it('applies multi-session set/delete atomically and rolls back every record on conflict', async () => {
    repository.save(initialRecord('one'), { expectedRevision: 0 })
    repository.save(initialRecord('two'), { expectedRevision: 0 })
    const result = await applySessionBatch([
    ...
    await expect(applySessionBatch([
  ...
  it('updates metadata buckets transactionally and reports integrity diagnostics', async () => {
    repository.save(initialRecord('one'), { expectedRevision: 0 })
    repository.save(initialRecord('two'), { expectedRevision: 0 })
    const updated = await updateSessionMetadataBucket('global', null, (current) => ({
      ...current,
  ...
  it('reads sessions-metadata as a metadata-only store without full repository snapshots', async () => {
    ...
    let calls = 0
    await saveSessionBody('global-one', { title: 'Global', messages: [{ role: 'user', content: 'hi' }] })
    await saveSessionBody('project-one', { title: 'Project', scope: 'project', projectId: 'p1', messages: [{ role: 'user', content: 'yo' }] })
    ...
    const all = await readSessionStateStore('sessions-metadata')
    ...
    expect(await readSessionStateStore('sessions-metadata', { scope: 'global' })).toEqual({ 'global-one': globalMetadata })
    expect(await readSessionStateStore('sessions-metadata', { scope: 'project', projectId: 'p1' })).toEqual({ 'project-one': projectMetadata })
    expect(await readSessionStateStore('sessions-metadata', { scope: 'project', projectId: 'other' })).toEqual({})
    ...
    await updateSessionMetadataBucket('project', 'p1', (current) => ({ ...current, 'project-one': { ...current['project-one'], taskStatus: 'idle' } }))
    expect(calls).toBe(0)
    expect(repository.findBySessionId('project-one').metadata).toMatchObject({ taskStatus: 'idle' })
  })

  it('reassembles split bodies on reads without the inline threshold', async () => {
    await saveSessionBody('small', { messages: [{ role: 'user', content: 'only one' }], title: 'Small' })
    ...
    expect(await readSessionStateValue('small')).toMatchObject({ title: 'Small', messages: [{ role: 'user', content: 'only one' }] })
  })
