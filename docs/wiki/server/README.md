...
- 统一校验 `busy_timeout=5000`、`foreign_keys=ON`、`journal_mode=WAL`、`synchronous=FULL`（§3.1 实测定案）；migration 使用 `BEGIN IMMEDIATE`、`PRAGMA user_version` 和同事务记录。
- 会话存储"当前架构"单一事实文档（F9 完成态基准：存储布局、phase 状态机、写/读路径、启动链现状、备份/降级工具与已知限制）见 [`docs/architecture/session-storage-current-architecture.zh-CN.md`](../../architecture/session-storage-current-architecture.zh-CN.md)；上述 F2/F6/F7/F8 设计文档保留为历史决策记录。
...
- `session-state-cutover.mjs` 执行 JSON→SQLite cutover（流式源 4 pass：双 summary 稳定性、备份 `createWriteStream` 流式写入 + 分块重读字节哈希校验、`replaceAllStream` 导入；pending 提交、pending/authoritative 完整性失败先 `rebuildIndex()` 自愈、复验仍失败才 fail-closed）并管理带 fencing/heartbeat 的维护锁；metadata-only 孤儿剔除并记入 diagnostics（`session_storage_state.diagnostic_json`，不阻塞迁移；`orphanDeletes` 经 mirror delete 队列在 drain 时物理清除 JSON 残留防 index 回流；restore/import 路径仍 400 拒绝）。
- `session-state-background-migration.mjs` 执行会话域后台迁移（设计：[`docs/architecture/session-storage-background-migration-design.zh-CN.md`](../../architecture/session-storage-background-migration-design.zh-CN.md)，含实施偏差记录）：`resolveSessionStateStartupRoute` 把 `json_authoritative`/`cutover_running` 路由到后台任务（不挡 READY；pending/authoritative 走 cutover 同步链）；任务状态机 importing（含收敛轮）→ idle-waiting → switching → done/failed/aborted，全程持维护锁（心跳续租；锁忙则 `aborted:'lock-busy'`，状态快照携带 lockOwner/Pid/Fencing 诊断）；桶级 `alignBucketStream` 对齐导入 + `deleteBucketRows` 孤儿清理（不 enqueue mirror）；收敛循环 digest 对拍；idle 期异步 v1 备份（复验通过可复用已登记 `backup_file` 并登记）；切换窗口在全局 persist 锁 + `acquireSessionJsonWriteBarrier`（drain 后 park 新写、FIFO 保序）内最终对拍 + `promoteAlignedSessionState`（json_authoritative→pending→authoritative 单事务）+ 刷新 service 相位缓存 + 空队列 drain 确认；`cutover_running` 残留在锁内复位（`phase.reset`）；`readSessionStateBackgroundMigrationStatus` 供 migration-status `sessionState.background` 域消费。
...
- `sqlite/database.mjs` 的 `runSharedSqliteQuickCheck()` 统一四个域的启动 `PRAGMA quick_check`（三个 repository 的 `verifyIntegrity({quickCheck:true})` 与 `storage.health({quickCheck:true})`）：进程内按数据库路径去重 + 数据库同目录 marker 文件（`quickforge-quick-check.marker.json`，7 天内跳过真扫），失败照抛且不写 marker 不缓存；`verifyIntegrity` 支持 `forceQuickCheck`（维护端点 `verify-session-integrity` 已强制真扫），环境变量 `QUICKFORGE_SQLITE_QUICK_CHECK=force` 全局强制真扫。安全性：WAL+synchronous=FULL 无 torn-write，quick_check 防磁盘 bit-rot，7 天周期+手动入口即可（详见 `docs/architecture/sqlite-storage-foundation.zh-CN.md` §6.1）。
