...
- 设置弹窗由 `hooks/useModelActions.ts` 组装多个 `SettingsTab`，包含 `lib/channels-settings-tab.ts` 的“渠道”页，用于管理本地外部应用 bridge（当前内置微信渠道，通过 `weixin-acp` 接入 `qf acp`，默认使用全局默认工作区，也可选择已有项目启动），底部包含 `lib/about-settings-tab.ts` 的“关于”页，用于展示 GitHub、检查 npm 更新并触发本机一键更新
