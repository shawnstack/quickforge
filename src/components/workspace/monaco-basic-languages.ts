/**
 * 静态引入 monaco-editor 全部 basic-languages 贡献，仅注册语言元数据与
 * Monarch 着色器的懒加载器；各语言的 tokenizer 按需动态加载。
 *
 * 只读查看器不需要 json / css / html / ts 语言服务 worker（补全、诊断、hover），
 * 因此不引入 `monaco-editor/esm/vs/language/*` 贡献，也不会请求对应 worker。
 * 注意：basic-languages 不含 `json`，JSON 文件以纯文本呈现。
 */
