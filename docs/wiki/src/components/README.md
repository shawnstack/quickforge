...
│   ├── GitToolsPinnedSummary.tsx   # 顶部工具栏置顶摘要：Git 工具、任务清单与智能体（运行中/已结束双小节）
...
- 顶部工具栏的 `GitToolsPinnedSummary` 保留 36×36 `List` 触发器，并泛化为当前会话“置顶摘要”：存在 Todo、运行中或最近结束的 Subagent 或 Git 仓库任一内容时显示（因此非 Git 会话也能查看前几类内容），Workspace Inspector 打开期间仍按既有规则隐藏。展开浮层不再显示顶部总标题/描述，只保留 absolute 定位在右上角的 X；实际分组按 Git 工具 → Todo → 智能体顺序渲染，首个存在的分组无顶部间距或分割线，后续存在的分组使用紧凑浅色分割线，标题行预留右侧空间避免与 X 重叠。Todo 复用当前消息分支最新合法 TodoWrite 完整快照，默认展示 3 项并可在浮层内展开全部；智能体分组标题 i18n 为「智能体 / Agents」，内分两个小节——「运行中」收集 `pendingToolCalls` 中的 `run_subagent` 调用（`extractRunningSubagentRuns`，仅由 tool call params 构建载荷）默认展开，行为弱色 spinner + 名称 + 任务副行、不显示耗时；「已结束 · N」排除 pending 后按终态时间最近优先取 3 项（`extractLatestTerminalSubagentRuns`），默认折叠，标题行整行点击切换（`aria-expanded` + ChevronRight/Down，关闭弹层恢复折叠、不持久化），展开后为 ✓/✗ 状态图标 + 名称 + 静态耗时 + 任务副行；两小节均只扫描当前 agent 的消息与 `pendingToolCalls`，不遍历全局 `subagentRunStore`，避免跨会话/回滚污染，空小节自动隐藏。点击运行中或已结束智能体行都会先关闭浮层，再复用 App 的 Workspace Inspector 请求链路打开或激活对应详情 Tab；Git 分组（标题 i18n「Git 工具 / Git Tools」）仅在当前项目为 Git 仓库时显示更改、分支与提交/推送入口。移动端使用视口内 fixed 滚动浮层和成对约束的 fixed 分支菜单 top/max-height，桌面端保持 overflow visible，避免现有分支子菜单被裁剪。
...
- `GitToolsPinnedSummary.tsx` 显示在顶部窗口工具栏中，收起态为摘要图标；展开后不显示顶部总标题/描述，只保留右上角 X，并按 Git 工具 → Todo → 智能体排列实际存在的分组（智能体组内分「运行中」与「已结束 · N」双小节，已结束默认折叠、标题行整行切换）。首组无顶部分割线，后续组使用紧凑浅色分割线；Git 组提供更改统计、分支操作和提交/推送入口。小屏 H5 与 Android WebView 使用标题栏下方、限制视口高度的固定浮层，支持点击外部或 Escape 关闭；Inspector 已提供移动全屏覆盖布局（GitTools 更改入口维持现状），小屏仍隐藏“更改”入口，桌面端仍可跳转 Changes/审查面板。
