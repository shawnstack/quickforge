...
- Files tab 通过后端 `/api/workspace/tree` 浏览当前项目文件；从 Files 中打开普通文件时，会提升为独立顶部 reader tab，Tab 标题显示文件名，内容区保留左侧工作目录树并在右侧使用 Monaco Editor / Markdown Reader 只读展示，普通代码文件关闭自动换行并在底部常驻横向滚动条；文件 reader tab 按项目保存路径，恢复时重新加载最新内容。Markdown 和代码产物进入 reader，HTML、SVG 和图片产物进入 Browser。
