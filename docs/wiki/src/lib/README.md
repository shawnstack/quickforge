...
**用途**: 在 `pi-web-ui` 中注册本地工具渲染器；`run_command` 运行中会显示终止按钮，通过 `/api/agents/:sessionId/abort-tool` 手动结束当前命令。
...
- `toolStartEventWithPartialResult()` / `upsertToolResult()` — 在运行中工具结果里保留计时、`sessionId` 和 `toolCallId`，用于前端展示耗时和终止运行中的 `run_command`。
