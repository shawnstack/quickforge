...
- `-EmitSummary`: prints JSON normalization summary to stdout (bytes, output lines, and `events` mode counters including `status_event_count`, `message_event_count`, `system_event_count`, `user_event_count`, `assistant_event_count`, `label_event_count_sum`, `label_sum_matches_message_count`, and `status_to_message_ratio_bps`).
