...
/** 工具行的直接容器：stage-body 动画壳层 inner 里的 step（工具组层级已移除）。 */
function rowsStepOf(group: FakeNode | null | undefined) {
  return group?.querySelector('.quickforge-process-stage-body .quickforge-process-body-inner .quickforge-process-step') as FakeNode
}
...
describe('process folding incremental suffix fold (full path, surviving group)', () => {
  it('appends new tool rows into the existing stage step without moving the rows it owns', () => {
    ...
    const step = rowsStepOf(group)
    const fingerprint = group?.dataset.quickforgeProcessFp
    expect(group?.parentNode).toBe(tree.content)
    // 工具组层级已移除：回合（无中间 Markdown）首段也包进 stage，工具行直接挂 step。
    expect(group?.querySelector('.quickforge-process-tools')).toBeNull()
    expect(step.children).toEqual([tree.tool1, tree.tool2])
    ...
    // 同一个组、同一个 step 容器：已有行节点一次都没有被搬动（搬动会重启
    // 行内 CSS keyframes），只有新行被增量插入。
    expect(group?.parentNode).toBe(tree.content)
    expect(tree.tool1.parentNode).toBe(step)
    expect(tree.tool2.parentNode).toBe(step)
    expect(tool3.parentNode).toBe(step)
    expect(step.children).toEqual([tree.tool1, tree.tool2, tool3])
  ...
  it('falls back to the full rebuild when the change is not a tool suffix append', () => {
    ...
    const assistant2 = tree.list.append(el('div', 'qf-assistant-message'))
    const thinking = assistant2.append(el('div', 'px-4 flex flex-col')).append(el('div', 'qf-thinking-block thinking-block'))
    ...
    const rebuiltStep = rowsStepOf(rebuilt)
    expect(rebuiltStep).not.toBe(step)
    expect(rebuiltStep.children).toEqual([tree.tool1, tree.tool2, thinking])
