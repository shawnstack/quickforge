...
- `panel-decoration/unreachable-strip.ts`（258 行）为持续不可达 Tier2 常驻条 controller（design-mockups/unreachable-notice.html 方案 A）：订阅 `subscribeSseConnectionState` 且自持 1s interval，当 reconnecting 且 `unreachable` 且距 `unreachableSince` ≥30s（无新广播也到点自动出现）时在 composer dock（`message-editor` 外层、`decorateEditor` 挂 `.quickforge-composer-dock` 的节点）之前插入 `role="alert"` 琥珀常驻条——位于滚动容器外、任何滚动位置可见：断开时长（`<60s` 显示「45s」，≥60s 显示「1m 05s」，tabular-nums）+「健康检查失败 · Xs 后自动重试」副文案 + 弹性空隙 +「立即重试」（`requestSseReconnectNow`）+「恢复指引」toggle；展开区精简为两行——按运行环境只显示对应动作（桌面应用 body class `quickforge-desktop-app` 或 `window.__quickforgeDesktopApp` → 重启应用；`isMobileShell()/isRemoteQuickForgeClient()`（mobile-server 导出不可用时回退 /Android/i UA）→ 检查主机与 Tailscale；默认 → 终端 `qf restart`）+ 日志路径行，不整列三种环境（用户反馈文字太多后由 5 行排序改为过滤）；恢复（connected/failed/非 unreachable）直接移除条并清 interval；展开状态在 controller 实例内记忆、decorate 周期 `sync()` 重挂后保留（元素被 Lit 重建后重新插回 dock 前，位置正确时不移动避免重启动画）；`destroy()` 退订清计时器。仅主聊天挂载（同 reconnect-notice）；导出 `UNREACHABLE_STRIP_AFTER_MS=30000` 供 Tier1 隐藏判断共用；文案全部 createElement/textContent，innerHTML 仅静态 SVG 常量
 - `panel-decoration/assistant-artifact-card.ts` 在最后一条已完成 assistant 回复底部挂载本轮文件产物卡片（对齐桌面截图的两类卡）：present_files 每个文件渲染独立单文件卡（类型 SVG 图标 + 文件名 + i18n「类别 · KIND」副标题 + 「打开▾」下拉）；write_file/edit_file 聚合为一张默认折叠的「N 个文件已更改 +X -Y」卡（同一文件一轮多次写入按路径合并为一行，多次写入 ± 取净变化 Σ加−Σ减、单次调用保留真实 hunk 计数，mergeChangedArtifactsByPath/dedupePresentedArtifacts 卡片层去重）（header role=button，点击/Enter/Space 切换 chevron 展开；展开态持久在宿主消息元素 dataset，跨 Lit 重建保留），展开后每行 = 类型图标 + 文件名/路径同行 + 会话级 +N/-N 统计 + 「审查」+「打开▾」。样式定稿（design-mockups/assistant-reply-file-card-v2.html）：卡片 `width:100%` 填满消息容器与对话正文同宽；± 统计红绿着色与应用 diff token 同源（light green-700/red-700、dark green-300/red-300），折叠头统计组（margin-left:auto）与行内统计均配 GitHub 风格 5px 比例条（段宽按 +N/−N flex 占比，`createDiffBar`）；打开/审查/撤销为 ghost 按钮（无边框 hover 浮底，撤销 hover 转红，按钮与行内 ± 统一 0.75rem 字号档）；明细展开用 grid-template-rows 0fr→1fr 动画（visibility 延迟切换防折叠区 Tab 聚焦，reduced-motion 关闭）。「打开▾」菜单项：预览打开（App `onOpenFilePreview` → `openArtifactPreview` 统一入口）与在文件管理器中显示（App `onRevealFile` → `openWorkspaceExternal(…,'explorer')`，复用既有 open-external 路由，服务端零改动）；菜单模块级互斥（`closeActiveOpenMenu`，removeArtifactCards 时统一清理监听）；菜单与共享撤销弹层（`floating-position.ts` 的 `positionFixedDropdown`）用 fixed 视口定位——右对齐触发器、clamp 进视口、下方不足向上翻（弹层 `-up` 箭头变体）、scroll/resize 即关——逃逸明细动画层 overflow:hidden 与消息列表 overflow-y-auto 的裁剪。行内「审查」经 App `onReviewFileChanges(path)` → `requestWorkspaceInspector({ kind:'review', view:'changes', path })`，review 请求协议新增可选 `path`（`workspace-types.ts`），WorkspaceInspector 请求 effect 直达 `openDiffTab` 单文件 Monaco diff（git 工作区口径，与 ± 统计的会话影子备份口径不同，源码注释已标注）。头部「撤销」按钮（复用 `panel-decoration/rollback-confirm-popover.ts` 共享确认弹层，自 message-actions 抽出）经 App `rollbackFilesFromArtifactCard` → `ServerAgent.rollbackFiles()` 调 POST `/api/agents/:id/rollback-files`，撤销会话级影子备份（恢复本会话修改文件、删除会话新建文件，幂等）；成功后按会话置灰为「已撤销」，新一轮文件产物（onArtifactsChange）重新武装，部分失败/异常走 addToast；readOnly 面板不传撤销/显示回调。幂等策略为签名跳过重建：卡片带 `dataset.quickforgeArtifactSignature`，decorate 周期计划签名一致时仅校正插入位置、不 remove+recreate——确认弹层/下拉菜单/展开态因此能在任意 DOM mutation 触发的重装饰中存活；签名变化才移除旧卡重建并先 `closeActiveOpenMenu`。卡片生命周期为「最近一个产物轮」：流式不清卡（纯 return 保留上一轮），findLastArtifactTurn 在完整 messages（含产物来源 toolResult.details，displayEntries 过滤掉它们，不能作提取源）从新向旧按 user 边界扫描取最近一个有 write/edit/present 产物的轮，经对象引用映射到 display 元素并挂到该轮最后一条 assistant——新轮无文件改动时旧卡保留、有产物时流式结束后换轮重建、整个会话再无产物轮才清除。
