...
│   ├── GoalSummarySection.tsx      # 两行 Goal 纯导航：目标 + 状态/验收计数，点击打开侧栏 progress
│   ├── goal-summary.css            # Goal 两行摘要导航样式
...
- Goal 生产 UI 以 `ServerAgent.state.goal` 为权威，职责分为运行条、摘要导航与侧栏详情：`panel-decoration/goal-control-strip.ts` 为 composer shell 首位的原生 DOM 运行条；`git/GoalSummarySection.tsx` 为两行纯导航；`workspace/GoalInspectorContent.tsx` 在 Workspace Inspector 内集中承接 progress（目标、验收、证据、范围、预算与阶段动作）和 edit（目标编辑）。旧 `goal-card.ts` controller 不再由 `ChatPanelHost` 挂载，公开导出与 `buildGoalCardViewModel` 纯投影保留兼容；字段仍经 `normalizeGoalState` 防御归一化，沿用既有视觉 token。
- 运行条显示状态、服务端已记录的累计时长（不以挂载时间或浏览器计时伪造实时用量；≥1 分钟按分钟向下取整、不足 1 分钟按秒），以及取消 / 暂停或继续 / 编辑三个 icon，动作按真实状态与 pending 禁用；终态整条不渲染。目标正文不再进入运行条（置顶摘要首分区与侧栏 progress 完整展示），取消确认展开时第二行文案收敛为单句「不回滚」说明。dirty 草稿不阻止安全 pause/cancel，继续等动作仍受共享 dirty 锁约束。节点原地更新、pause/resume 复用按钮，取消走内联确认。pause 直接请求真实后端，`pausing` 不是原型的 800ms 假延迟；编辑 icon 导航至侧栏 edit。
