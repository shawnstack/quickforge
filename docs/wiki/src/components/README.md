...
- `panel-decoration/todo-write-summary.ts`（311 行）负责 TodoWrite 最新快照的规范化、QuickForge/OpenCode 当前消息分支提取、计数与 Composer Dock 正常流摘要 controller；摘要位于 `message-editor` 前，展开时自然压缩消息区、长列表内部滚动，复用既有轻盈内嵌工具视觉，不引入新的视觉范式，因此无需更新 `DESIGN_LANGUAGE.md`
