...
export const goalReportTool = {
  ...
  description: 'Record the goal plan and its progress. Planning is read-only through the entire turn: submit action="plan" with verifiable criteria, scope and summary, then end the turn. Execution starts automatically after normal run end and durable persistence, without plan confirmation. Use action="progress" for evidence updates and action="blocked" with a concrete verification limitation or necessary question when unable to verify. The legacy action="needs_review" records blocked, never a fabricated pass or required human sign-off. Use action="complete" only when every required criterion has evidence bound to a real successful tool result (goal_report/todo_write/ask_user do not count). Completion requires normal run end and durable persistence. Goal has an iteration budget but no cumulative time limit; tool approvals, questions and individual tool timeouts remain in force.',
  parameters: Type.Object({
    action: Type.Union([
      Type.Literal('plan'),
