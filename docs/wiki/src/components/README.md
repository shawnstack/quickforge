...
- 位于「设置 → 账户与云服务」，顶部配置独立受管 QuickForge Cloud 服务连接和总开关（Cloud URL、来源、health/ready 测试和保存），不展示 Provider Key，也不写入自定义 Provider。总开关默认关闭，保存 URL、连接测试或重建身份都不会自动开启。
- 关闭时保留 URL、本地身份/账户摘要和退出入口，隐藏并停止远程访问、云模型、额度、设备、登录入口及 Device Flow 轮询；重新开启后再按当前状态加载远端详情。
- 同页展示可选云身份、剩余额度、重置/到期时间、公开模型目录和连接设备；启用时一次刷新同步请求 config/status/usage/installations/models，其中 usage/installations/models 独立提交结果：单项失败会清除该项旧数据并在对应区块提供重试，其余成功内容继续展示。
