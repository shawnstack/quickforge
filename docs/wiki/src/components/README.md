...
`panel-decoration/goal-iteration-divider.ts` 读取消息持久化 `details.quickforgeGoalIteration`，在消息根末尾、过程折叠外幂等装饰两侧细线与灰色 SVG 图标文字。`kind` 区分 planning / execution，legacy 无 kind 按 execution；未知 kind 或无效 iteration 移除旧装饰。规划 iteration 可为 0、只显示「规划」不显示轮数；仅正常提交 plan 轮末且保存成功的 running marker 显示「计划已就绪」，其他规划结果按实际失败/暂停/阻塞/取消/待规划显示，非成功不沿用成功勾号。执行按真实 outcome/blocker 区分继续执行、完成、失败重试、暂停、阻塞、取消、预算耗尽与历史人工复核；执行 error/阻塞/取消轮与规划失败同样使用警示叹号图标，完成/继续/暂停/人工复核与可恢复的预算阻断保留成功对勾。

分隔线不依赖当前 Goal、不新增聊天消息；completed 同样须服务端正常轮末持久化成功，不把工具成功当最终完成。自动 completed 不提供 accept，显式 needs_review 转 blocked；旧记录只展示历史事实。ServerAgent 的 sparse/full marker 快照按索引和身份合并、后到消息重放，并以本地 `message_metadata_updated` 通知 `ChatPanelHost` 刷新窗口消息与装饰；该事件不清 streaming process groups、不恢复 composer draft，也不放宽正文 streaming 门禁。只有 marker 实际变化才推进消息 watermark，单独 goal_updated 不推进。详见 [元数据同步契约](../lib/README.md#goal-分隔线元数据实时同步)。

视觉顺序由 `src/index.css` 保证：`.quickforge-goal-iteration-divider` 的 `order: 3` 排在消息操作栏 `order: 2` 之后，即正文/过程 → 操作栏 → 阶段分隔线；不改变根末尾装饰、内部 user fallback 或空 assistant source 的直接分隔线保留契约。

考虑 SVG 后复用现有细线/SVG，不新增图或视觉模式。`goal-iteration-divider` / `message-actions` / `server-agent` 自动回归不等于真实浏览器验收；窄屏、主题、焦点、屏幕阅读器、真实刷新与现场模型流程仍需实测。
...
│   ├── file-rollback-state.ts      # 撤销弹窗状态机：createFileRollbackController / createTurnRollbackController（409→conflict、网络错→unconfirmed 禁再执行）(193 行)
...
├── git/
│   ├── GitBranchMenu.tsx           # 标题栏 / 置顶摘要（旁挂）/ 提交推送对话框中的分支搜索、切换下拉
│   ├── GitCommitPushDialog.tsx     # Git 提交 / 提交并推送 / 推送弹窗，支持 AI 生成提交信息
│   ├── GitToolsPinnedSummary.tsx   # 顶部 List 常驻；桌面 closed/capsule/panel 三态可拖动摘要，胶囊用 Maximize2 弱提示展开，移动端固定两态 List 弹层；Goal 分区置顶 + 任务清单 + 智能体双小节 + Git 工具
│   ├── GoalSummarySection.tsx      # Goal 带标题分组摘要：标题「Goal/目标」+ passed/total 计数 + 规划 criteria 只读行 + 打开侧栏 progress 的导航按钮
│   ├── goal-summary.css            # Goal 分组摘要样式
...
├── workspace/
│   ├── WorkspaceInspector.tsx      # 右侧统一工作区检查器，顶部 Tab 支持拖动排序；含文件/审查/终端/浏览器/subagent/Goal 运行时详情
│   ├── GoalInspectorContent.tsx    # Goal progress/edit 子视图：独立滚动内容区+底部固定动作栏、tone 状态行与 ? 说明浮层、预算计量条、验收 badge、安全保存和冲突提示
│   ├── goal-inspector.css          # Goal 侧栏滚动布局、分段导航、状态行/浮层/计量条/动作栏样式
...
- 撤销接线（per-turn-artifact-cards）：轮级撤销是产物卡唯一入口；消息装饰 deps 为 `onRollbackTurn(turnIds)` / `rolledBackTurns`（`ReadonlySet<轮键>`，轮键 = `turnRollbackKey(turnIds)` = `join('|')`，进产物卡签名驱动「已撤销」态），并以 `getArtifactMessages: () => agent.state.messages` 显式传全量消息给产物卡提取（不被 windowed/displayEntries 裁剪）；readOnly 面板不传 `onRollbackTurn`。App.tsx 持有 `rolledBackTurns` 状态，轮级弹窗经 `rollbackTurnFromArtifactCard(turnIds)` 打开 `TurnRollbackDialog`（`fileRollbackTarget.turnIds` 携带该轮全部 turnId，含重试 run 的 id；重试追加新 turnId 后轮键变化、旧键残留无害——新产物本就应重新武装按钮）。会话级「撤销全部」入口已移除：`FileRollbackDialog` session 模式组件与 `ServerAgent` 的 `rollbackFiles` / `getFileRollbackPreview` / `rollbackFile` API 保留但无挂载入口。
- 主聊天 Composer 在「完全访问权限」后、Plan 前显示当前会话运行中的 Subagent 指示器：数据只遍历当前 agent 的 `state.pendingToolCalls`，再按 toolCallId 从全局 `subagentRunStore` 取 `status=running` 快照，避免其他会话污染；0 个时移除。Trigger 为静态 Bot 图标 + 右上角 emerald 数量角标（无动画），重复装饰复用 icon/badge DOM 仅更新数字。点击打开 body-level fixed 运行列表（名称、任务、递增耗时），菜单项按 runId 就地 diff 更新（复用元素、仅更新变化字段、乱序才移动、消失即删除，不整体 replaceChildren），流式输出期间 hover 不再闪断；trigger 元素被宿主重建时同步菜单 ownerTrigger 引用，菜单保持打开。点击具体项派发既有 `quickforge:open-subagent-run` 事件并复用 Workspace Inspector 的 Subagent 详情 Tab。菜单与 Composer 其他弹层互斥，支持外部点击/Escape 关闭、滚动/缩放重定位、DOM 重挂幂等及卸载清理；Side Chat、只读页、禁用控件不显示。
- Composer 底栏控件为完全去框形态（方案 A，design-mockups/composer-borderless.html）：`+`、模型选择与 Agent 权限按钮均无边框、无浅色填充，静默时次级文字色；hover/展开统一中性浅色底，「完全访问」模式仅文字/图标为琥珀 #d97706。思考等级从模型按钮的「· 等级」后缀拆为独立选择器 `panel-decoration/thinking-level-controls.ts`（大脑 icon + 等级文字 + 下拉箭头，位于模型选择右侧、发送左侧；`model.reasoning` 为 false 时自动隐藏），点击向上弹出五档菜单（关/低/中/高/超高，样式与 Agent 权限菜单一致、与其他 Composer 弹层互斥）；选择经 `onThinkingLevelChange` 回调写 `agent.state.thinkingLevel`、调 `updateThinkingLevel` 同步服务端并重跑装饰。模型选择弹层（custom-model-selector 已移除 thinking 区与 `thinkingLevel`/`onThinkingLevelSelect`/`showThinking` 选项）桌面端为单一列表（「模型」头部 + 全部模型项勾选态 + 设置入口，不再有当前模型行→hover 子菜单二级结构；空列表显示「未添加模型」提示），移动端 sheet 同步只保留模型列表；切到非推理模型时由调用方（useModelActions / SharedConversationPage）自动把思考等级归零。模型按钮为纯文字 + 下拉小箭头、不设 max-width（窗口缩放不截断模型名；CSS `::after` chevron，紧凑/移动端隐藏文字与箭头、以 `::before` 盒形 icon 呈现），思考按钮在紧凑/移动端收成 icon-only；icon-only 形态下按钮行改 `justify-content: flex-start` + `gap: 0.5rem`，全部图标控件（+、权限、模型、思考…）连续排列且行内/行间间距统一，发送/停止按钮 `margin-left: auto` 保持右缘。
...
- Goal 生产 UI 以 `ServerAgent.state.goal` 为权威，职责分为运行条、摘要导航与侧栏详情：`panel-decoration/goal-control-strip.ts` 为锚定 message-editor 紧前的原生 DOM 运行条；`git/GoalSummarySection.tsx` 为带标题分组摘要；`workspace/GoalInspectorContent.tsx` 在 Workspace Inspector 内集中承接 progress（目标、验收、证据、范围、预算与阶段动作）和 edit（目标编辑）。旧 `goal-card.ts` controller 不再由 `ChatPanelHost` 挂载，公开导出与 `buildGoalCardViewModel` 纯投影保留兼容；字段仍经 `normalizeGoalState` 防御归一化，沿用既有视觉 token。
- 运行条显示状态、服务端已记录的累计时长与取消 / 暂停或继续 / 编辑三个 icon，动作按真实状态与 pending 禁用；终态整条不渲染。时长以服务端结算值为最终基准，快照之间由 1s interval ticker 插值当前轮流逝时间（每次快照到达重置锚点：显示 `usage.activeDurationMs` + 自快照到达起的时间），让「已记录 N 秒」实时递增（用户要求时长可见递增；此前规划轮进行中时长冻结在 0 秒）；goal 移除、转 terminal 或 detachRoot 即停并清理，再挂载时重新锚定。≥1 分钟按分钟向下取整、不足 1 分钟按秒。目标正文不再进入运行条（置顶摘要首分区与侧栏 progress 完整展示），取消确认展开时第二行文案收敛为单句「不回滚」说明。dirty 草稿不阻止安全 pause/cancel，继续等动作仍受共享 dirty 锁约束。节点原地更新、pause/resume 复用按钮，取消走内联确认。pause 直接请求真实后端，`pausing` 不是原型的 800ms 假延迟；编辑 icon 导航至侧栏 edit。规划类状态（planning/awaiting_confirmation）的状态 icon 为 list-todo 清单 SVG（复选框+勾+三条线），`goal-control-strip.ts` 与 `goal-card.ts` 两处 `STATUS_ICON.info` 副本同步；`isGoalSpinning` 旋转集合已移除 planning——清单图标静止，仅忙碌状态（running/verifying/awaiting_input/awaiting_approval/pausing）的 icon 旋转（strip 与卡片共享该语义，`prefers-reduced-motion` 下全部静止）。运行条宽度 `fit-content`（上限 100%），随内容自适应收缩不再占满 composer shell 全宽，左右 auto margin 使其在 composer 内水平居中。
- 置顶摘要 Goal 首分区在 Git/任务/智能体之前，保留 goal-only 挂载与胶囊 Goal 首段。`GoalSummarySection` 为与 Git/Tasks 分组一致的带标题分组（`section aria-labelledby="pinned-goal-title"`，标题「Goal/目标」（i18n `pinnedGoalTitle`）+ passed/total 计数），组内按序渲染规划 criteria 只读行（状态 icon：passed→CheckCircle2 绿 / failed→XCircle destructive / needs_review→Clock3 琥珀 / pending→Circle 弱化，passed 加 line-through 弱化文本），保留整行点击打开侧栏 progress 的导航按钮（GoalIcon + 状态词 + 目标正文）；不内嵌编辑器或阶段动作，样式在 `git/goal-summary.css`。
...
- 预算耗尽时，运行条的继续入口导航到 Inspector progress，不直接派发旧 resume；`GoalInspectorContent` 提供额度内联确认（带边框/浅底/内边距的视觉容器），预算 usage/上限由双行计量条结构化展示（null 时长无预算可比，整行隐藏不显示「无限」及累计用时；仅有限时长展示计量条，旧数值时长耗尽判断不靠整分钟取整），确认组只保留本次仅耗尽轮次追加配置轮次（默认 20，增量读自 `goal-settings` 设置）、移除旧时间上限的说明一行（goalBudgetGrant）与仍不足提示，原毫秒拼接文案已移除；按钮为主 goalConfirm + ghost goalKeepWorking。确认绑定打开确认行时的旧 goalId/revision，快照变化需重新确认，pending/dirty 禁止提交。请求前严格预检，失败后对账而非盲重发；关闭侧栏只取消未发送 POST，已发送等待结算，不能宣称撤回。额度和执行状态由服务端决定，有计划即可恢复 running，不再要求 confirm。沿用现有内联确认/焦点模式，真实浏览器视觉、IME/焦点尚未验收；回归见 `tests/frontend/goal-budget-inspector.test.ts`、`goal-control-strip.test.ts` 与 `server-agent.test.ts`。
...
- 消息回滚、分叉、复制功能
- 顶部分支徽标的 git status 探测经 `workspace-api.ts` 的 `getGitStatus` 发起，带 `{ light: true }`（只消费 `isGitRepository`/`branch`，独立缓存键，不与标题栏 full 请求合并）：请求带 20s 超时（TimeoutError 中止，防止大仓库慢查询钉死 HTTP/1.1 同源连接池），卸载或 `gitProjectId`/`revision` 变化时通过 AbortController 立即中止，服务端在客户端断开时会中止 git 子进程（详见 server 路由文档）；被中止的请求静默处理，不写警告日志。App.tsx 标题栏刷新同样中止上一请求并在项目 scope 切换时立即取消。同一 `projectId` 的在途请求按引用计数共享（同一项目最多 1 条连接，任一调用方 abort 只结束自己的等待，最后一个等待者退出且请求仍在途时才中止共享请求），成功结果另有 1s 短 TTL 缓存（`GIT_STATUS_CACHE_TTL_MS`，`{ force: true }` 绕过，git 写操作发出时即失效）；WorkspaceInspector 手动刷新与 App 标题栏「工具执行结束后」的刷新都传 `force`（避免显示写操作前的快照），顶部分支徽标探测与 Inspector 首次加载复用缓存。App.tsx 的 `onProjectsChanged`（切回窗口 / 跨标签页通知）走 `useProject` 的 15s 项目列表缓存，不再强制刷新 `/api/project`。
- 顶部工具栏的 `GitToolsPinnedSummary` 泛化为当前会话“置顶摘要”：存在 Goal、Todo、运行中或最近结束的 Subagent 或 Git 仓库任一内容时显示（因此非 Git 会话也能查看前几类内容）。Workspace Inspector 打开时仅在与其侧栏布局一致的真实桌面场景（`>=1024px` 且非 `mobileShell`）把摘要置为 `suspended`，组件保持挂载并完整保留 closed / capsule / panel、拖动位置、Todo 展开与智能体折叠状态；toolbar root 与 desktop fixed widget 同时使用 `hidden` + `inert` + `aria-hidden` 真正退出布局、输入、Tab 与辅助技术，不遮挡 Inspector。`<1024px` 与 `mobileShell` 仍沿用原卸载/全屏 overlay 行为。暂停期间不安装 outside pointerdown/Escape，也停止 resize/布局解析、ResizeObserver 等副作用；进入暂停只结束 drag 并清 window pointer listeners/capture/body userSelect/drag 与 responsive rAF，关闭 branch menu 和待执行 focus，不取消用户已明确触发的 160ms close timer，也不通过可被 cleanup 取消的 rAF 归一化 closing/mounted；因此隐藏期间 close timer 会自然完成 `closed => unmounted`，组件最终卸载时既有 cleanup 仍会清 timer。其余 position/capsule/expanded 等状态不清且不自动回焦。关闭 Inspector 后复用统一方向性布局策略：横向保持 12px 安全区；panel 优先保留现有 top，从该 y 到 viewport bottom 12px 作为动态 max-height，让 flex 内容区滚动，只有下方连 180px 最小可用面板高度都不足时才向上移到刚好容纳该高度；极矮 viewport 则使用全部安全区域。恢复不调用 open/minimize、不抢焦点。PanelRight 的打开分支与摘要内 Git Changes/智能体入口共享“未来打开 Inspector 是否具备 suspension/preserve 能力”的判定：真实桌面侧栏保留摘要，`<1024px`/`mobileShell` 则在 `setWorkspaceInspectorOpen(true)` 前先清 `expanded`，保持原 overlay 行为；PanelRight 的关闭分支不修改摘要。Commit/Push 非 Inspector 路径始终先关闭摘要。顶部 `List` 按钮在桌面和移动端始终保留：桌面浮动摘要关闭时点击直接打开完整面板，面板或胶囊存在时点击完全关闭到仅顶部按钮，并通过 `aria-pressed` / `aria-expanded` 表示浮动摘要是否存在。桌面 `md+` 且非 `mobileShell` 使用 `expanded ? panel : capsuleVisible ? capsule : closed` 三态：完整面板标题栏提供独立 `Minimize2`“缩小为摘要”和 `X`“完全关闭”；只有显式 Minimize 会把 panel 变为 capsule，缩小后在当前浮动位置显示摘要胶囊，点击胶囊主体原位展开；主体末端使用 `Maximize2` 作为与 panel 标题栏 `Minimize2` 配对的展开提示，放在约 28px 的透明圆形视觉槽中，默认弱化、主体 hover/focus 时克制增强；该槽仍是主体 button 内的 `aria-hidden` 非独立提示，不新增第三个按钮。胶囊右端独立约 32px `X` 按钮（阻止冒泡）完全关闭。桌面 panel/capsule 点击外部都保持当前形态，desktop Escape 无摘要级动作且不拦截事件；顶部 List 与两种 X 继续完全关闭。mobile/mobileShell 仍由外点/Escape 关闭，无 capsule/minimize。branch menu 桌面改为从 panel 旁边旁挂弹出（不新增摘要级嵌套 dismiss 设计），摘要外点 stay 不再意外改变 panel/capsule。胶囊按实际存在字段统一数组渲染三类语义段：`ListTodo + completed/total`、`FileDiff + +N/−N`、单一 `Bot + running/finished`，避免运行中/已结束重复 Bot 段；只有 Git 仓库但无有效数字时回退 List。第一段前无分隔，后续段使用主题自适应 `bg-foreground/15` 浅黑分隔线，ARIA 只拼实际显示段并用中英文完整说明 running/finished。桌面 panel↔capsule 始终复用同一个 fixed widget（`data-mode="panel|capsule"`），面板和胶囊内容叠放交叉淡化；panel/capsule 子层使用 `transform-origin: top right`，与顶部/右侧视觉锚点一致，避免 scale 从中心造成视觉上浮。`ResizeObserver` 以目标节点不受 transform 影响的布局尺寸（优先 `offsetWidth` / `offsetHeight`，布局值不可用时才回退 rect）写入 CSS variables；panel 展开态另读 `scrollHeight + (offsetHeight - clientHeight)` 保存包含边框差值的自然高度，避免 panel↔capsule morph 少 2px；动态 max-height 只在 panel mode 更新并写入变量，不让 capsule 路径产生无意义 state。拖动期间 observer 使用 `dragRef.current.current` 作为布局 position，而非尚未收敛的拖动起点 `positionRef`，因此跨越“保留 top / 上移到最小高度”边界时不会被 observer 抢写旧 max-height；拖动结束、取消、暂停与响应式清理仍走统一 finish/cleanup 收敛。panel 本体依赖有效的 auto height + max-height，已删除无效 `height: min(max-content, ...)`。外层宽高保持 220ms `cubic-bezier(.22,.8,.24,1)` 过渡，closed↔浮层以 160ms opacity/scale 进退。拖动时通过 `data-dragging` 关闭形态过渡，left/top 不参与 transition；`prefers-reduced-motion` 下全部形态/内容动画关闭。位置只保存在组件生命周期，不写 localStorage。胶囊可从任意位置拖动，展开面板只允许约 40px 标题栏拖动；`pointerdown` 后临时由 `window` 跟踪同 `pointerId` 的 move/up/cancel，达到 4px 阈值前即使移出元素也不会丢失结束事件，只有真实越过阈值才设置 pointer capture、禁用文本选择并抑制随后的 click，普通静止点击仍由胶囊 button 原生 `onClick` 展开；拖动用 rAF 更新 fixed left/top，finish/cancel、响应式降级与卸载统一清理 window 监听、可选 capture、rAF 和 body 样式。panel/capsule 形态切换、window resize、拖动过程/结束与 Inspector resume 都复用上述统一布局安全策略；不会因目标 panel 的整屏自然高度把 y 向上 clamp。首次定位仍可使用 rect 的 x/y 锚点，确保最终 panel/capsule 尺寸与动态 max-height 正确。首次 desktop 定位由 `App` 在主对话 `<header>` 上提供稳定 React ref，组件只在 `positionRef.current` 尚不存在的首次分支读取该 header rect：`y = ceil(header.bottom) + 10px`，`x = header.right - targetWidth - 12px`，让胶囊/面板自然落在对话顶部栏下方并从主内容右缘内缩；header 不可用时先回退 toolbar root rect，toolbar root 也不可用时再回退当前 widget rect，最终仍通过 12px viewport 安全区布局。该锚点不依赖 querySelector、Tailwind selector 或固定 titlebar 高度，浏览器/Electron、字号与侧栏宽度变化由真实 rect 自然适配；用户拖动后、panel/capsule 切换、window resize 与 Inspector suspend/resume 都只解析现有 position，不再重读 header。展开面板内容主体始终 `overflow-y-auto overscroll-contain`，普通内容不会越过受限 panel；Git 分支菜单 desktop 不在 panel 内容流中向下展开，而是作为 widget 直接子节点从 panel 旁边弹出：默认左侧（`right-full mr-1`），左侧空间不足 340px 时翻到右侧（`left-full ml-1`），top 以分支行与 widget 的 rect 差对齐，max-height 受 viewport 12px 安全区约束（22rem 上限，内部列表自身滚动），打开时测量一次，拖动/形态切换/收起即关闭。实际分组按 Goal 分组（带标题 + criteria 只读行 + 导航按钮）→ Git 工具 → Todo → 智能体顺序渲染，首个存在的分组无顶部间距或分割线，后续存在的分组使用紧凑浅色分割线。Todo 复用当前消息分支最新合法 TodoWrite 完整快照，默认展示 3 项并可在浮层内展开全部；智能体分组标题 i18n 为「智能体 / Agents」，内分两个小节——「运行中」收集 `pendingToolCalls` 中的 `run_subagent` 调用（`extractRunningSubagentRuns`，仅由 tool call params 构建载荷）默认展开，行为弱色 spinner + 名称 + 任务副行、不显示耗时；「已结束 · N」排除 pending 后按终态时间最近优先返回全部（防御上限 `MAX_TERMINAL_SUBAGENT_RUNS`=100，`extractLatestTerminalSubagentRuns` 返回轻量载荷），标题计数为真实总数，默认折叠，标题行整行点击切换（`aria-expanded` + ChevronRight/Down，关闭弹层恢复折叠、不持久化），展开后为 ✓/✗ 状态图标 + 名称 + 静态耗时 + 任务副行；两小节均只扫描当前 agent 的消息与 `pendingToolCalls`，不遍历全局 `subagentRunStore`，避免跨会话/回滚污染，空小节自动隐藏。点击运行中或已结束智能体行会复用 App 的 Workspace Inspector 请求链路打开或激活对应详情 Tab；Git 分组（标题 i18n「Git 工具 / Git Tools」）仅在当前项目为 Git 仓库时显示更改、分支与提交/推送入口。`<md` 与 `mobileShell=true` 明确降级并完整保留原 List trigger、`fixed inset-x-2 top-14` 可滚动弹层及分支菜单 fixed top/max-height 配对，不渲染胶囊或桌面缩小按钮、不挂二维拖动 handler，也不设置 `touch-action:none`；进入该非 desktopDraggable 分支时清理 desktop position/capsule/mounted/closing 状态但不修改外部 `expanded`，因此移动端已展开 panel 不会被误关，返回桌面也不会残留幽灵胶囊；X/外点/Escape 关闭。
- TodoWrite 任务摘要由 `panel-decoration/todo-write-summary.ts` 的 controller 驱动：摘要是 Composer Dock 内、`message-editor` 前的正常流 sibling，不进入历史工具过程 DOM，也不覆盖消息或输入框；展开时自然占用 Dock 高度并压缩上方消息区。收起时收缩为居中的进度胶囊（环形进度 + 紧凑 `完成/总数` 计数 + 小箭头），展开时胶囊经 CSS 过渡（flex-grow 数值插值生长为整行、圆角/背景/标题与双统计交叉淡化、grid-rows 0fr→1fr 列表高度、列表项阶梯淡入、进度弧 dashoffset）还原为完整列表；动效全部由 `data-expanded`/`data-complete`/`data-running` 与 body 的 `hidden` 属性驱动（`transition-behavior: allow-discrete` 延迟 display:none 至收起动画结束），`prefers-reduced-motion` 下全部关闭。无有效 Todo 时不显示；首次取得含未完成项的有效快照时自动展开，用户手动展开/收起状态会在后续仍含未完成项的新快照中保留，并短暂显示“已更新”；任一新快照全完成时环交叉淡化为对勾并自动收起，但用户仍可手动重新打开；成功空数组清空或消息回滚到不存在有效快照时移除摘要并重置状态。长列表在摘要内部滚动，桌面约显示 4 项、移动端约显示 3 项。editor 或 Composer shell 被重建时 controller 会按当前快照自愈重挂；`readOnly` 页面没有 Composer Dock 时不显示。`ChatPanelHost` 在消息装饰刷新时调用 `update()`，卸载时调用 `cleanup()` 清计时器、观察器、点击监听和 DOM。基础系统提示词中的 `todo_write` 计划规则位于 `For project tasks:` 专属段之外，因此所有实际具备该工具的非简单多步骤任务（包括普通全局会话）都适用；UI 仍严格按有效 Todo 内容挂载，不为无内容会话常驻空入口。
- Composer Dock 的 sibling 顺序为：任务摘要 → 消息队列面板（存在时）→ Goal 运行条 → command/file 临时建议菜单 → `message-editor` → stats；临时建议菜单始终紧邻输入框，Goal 运行条锚定建议菜单（无菜单时锚定 editor），任务摘要出现时运行条仍紧贴输入框。任务摘要与消息队列面板互为合法相邻 sibling（锚点规则容忍彼此，`quickforge-msg-queue`/`quickforge-goal-strip` 是任务摘要的可接受后继，`quickforge-goal-strip` 亦是队列面板的合法后继；运行条是装饰序列最后写入者，被挤开后当周期末自愈），避免每个装饰周期交换位置。任务摘要插入、展开或收起会改变 Composer 布局，因此 Slash invocation overlay 同时观察 textarea 与 `.quickforge-composer-shell` 的尺寸变化并重算几何；两路 observer 在重建与卸载时成对 cleanup。
- 消息队列（排队/插队）由 `src/lib/message-queue.ts`（纯函数队列操作、localStorage 持久化、steer 客户端）+ `panel-decoration/message-queue.ts`（面板 controller）组成：流式期间 textarea capture-phase keydown 拦截 Enter，把输入加入本地队列并把占位符切为「继续输入以排队后续修改」（在 decorateEditor 之后覆盖默认占位符）。回合自然结束（`agent_end` 且非 aborted/error）且队列非空时延迟 250ms 取队头作为普通 prompt 自动发送直至清空；`aborted`/`error` 结束（含停止按钮与 Escape 中断）则暂停并显示恢复入口。队列项支持行内编辑、删除与「立即」插队——经 `ServerAgent.steer`（乐观显示：点击即在对话流中追加该 user 消息，服务端在当前工具轮结束后注入同一消息并按 role+timestamp 原位对账，HTTP 失败回滚乐观副本）注入进行中回合（能力位 `messageSteering` 在 QuickForge 主会话开启、Side Chat 关闭；steer 不可用或未流式时点击退化为置顶立即发送）。队列项支持拖拽排序：行首 ⠿ 手柄触发指针 mini-sortable（6px 阈值激活、原位半透明占位 + body 级克隆幽灵跟随、越过相邻行中线占位实时换位、列表边缘自动滚动），move/up 监听挂 window capture（手柄级监听会因指针捕获不生效而丢失抬起）；拖拽会话为每控制器持有，流式期间 decorate 周期触发的重渲染在拖拽进行中挂起（会话结束时统一从权威 items 重建，取消/中断的拖拽也因此复位行序），仅控制器卸载时取消会话，落点经 `moveQueuedMessage` 提交并照常持久化；不支持 React 版 @dnd-kit（控制器为原生 DOM）。行内编辑跨 decorate 重建保值/保焦点/保光标（仅当编辑项未变时）。状态持久化到 localStorage（`quickforge:message-queue:v1`），卸载即保存、挂载时按 sessionId 恢复；上限 20 条、单条 2000 字符；自动发送失败回退队头并暂停，steer 失败仅记 warn 并保留该项。
...
- 左侧聊天列表面板；移动端抽屉为 100% 全屏宽（`w-full`），不再受 85vw 上限限制；桌面侧栏默认/最小宽度为 304px、最大宽度为 520px，侧栏通过 `ResizeObserver` 回报实际宽度，供右侧 Workspace Inspector 在 1200px / 75vw 上限之外同时保留左栏、1px 分隔线和 440px 对话区
...
- 创建自定义 Agent，配置系统提示词、模型、思考等级、工具白名单、运行时间、工具调用次数和是否启用为 sub agent；可分别勾选“允许使用 MCP 工具”和“允许使用 Agent Skills”（默认关闭）；最大运行时间在界面按分钟填写（至少 1 秒，即 `1000/60000` 分钟，且不超过 60，支持小数），API 与配置文件仍使用毫秒字段
- Subagent 开启 MCP/Skills 后，只继承主 Agent 当前实际工具集中的对应能力，不额外扩展全局工具；MCP 继续遵循主会话审批规则，子 Agent 不可递归调用 subagent
...
- Composer 内独立 `@` 文件引用控制器，只在当前 QuickForge 项目、可编辑会话启用；分享页、无项目和只读页禁用，插件不会进入 `@` 结果。
...
- `panel-decoration/todo-write-summary.ts`（360 行）负责 TodoWrite 最新快照的规范化、当前消息分支提取、计数与 Composer Dock 正常流摘要 controller；摘要位于 `message-editor` 前，收起为居中进度胶囊、展开为整行标题 + 列表（形态过渡与动效全部在 CSS），展开时自然压缩消息区、长列表内部滚动，复用既有轻盈内嵌工具视觉，不引入新的视觉范式，因此无需更新 `DESIGN_LANGUAGE.md`
...
- `panel-decoration/unreachable-strip.ts`（258 行）为持续不可达 Tier2 常驻条 controller（design-mockups/unreachable-notice.html 方案 A）：订阅 `subscribeSseConnectionState` 且自持 1s interval，当 reconnecting 且 `unreachable` 且距 `unreachableSince` ≥30s（无新广播也到点自动出现）时在 composer dock（`message-editor` 外层、`decorateEditor` 挂 `.quickforge-composer-dock` 的节点）之前插入 `role="alert"` 琥珀常驻条——位于滚动容器外、任何滚动位置可见：断开时长（`<60s` 显示「45s」，≥60s 显示「1m 05s」，tabular-nums）+「健康检查失败 · Xs 后自动重试」副文案 + 弹性空隙 +「立即重试」（`requestSseReconnectNow`）+「恢复指引」toggle；展开区精简为两行——按运行环境只显示对应动作（桌面应用 body class `quickforge-desktop-app` 或 `window.__quickforgeDesktopApp` → 重启应用；`isMobileShell()/isRemoteQuickForgeClient()`（mobile-server 导出不可用时回退 /Android/i UA）→ 检查主机与 Tailscale；默认 → 终端 `qf restart`）+ 日志路径行，不整列三种环境（用户反馈文字太多后由 5 行排序改为过滤）；恢复（connected/failed/非 unreachable）直接移除条并清 interval；展开状态在 controller 实例内记忆、decorate 周期 `sync()` 重挂后保留（元素被 Lit 重建后重新插回 dock 前，位置正确时不移动避免重启动画）；`destroy()` 退订清计时器。仅主聊天挂载（同 reconnect-notice）；导出 `UNREACHABLE_STRIP_AFTER_MS=30000` 供 Tier1 隐藏判断共用；文案全部 createElement/textContent，innerHTML 仅静态 SVG 常量
 - `panel-decoration/assistant-artifact-card.ts` 按轮挂载文件产物卡片（对齐桌面截图的两类卡；per-turn-artifact-cards：每轮一张，挂该轮最后一条已完成 assistant 回复底部，只显示该轮新增产物）：present_files 每个文件渲染独立单文件卡（工作区文件管理同源的 Material 文件图标（fileIconUrl 按路径解析）+ 文件名 + i18n「类别 · KIND」副标题 + 「打开▾」下拉）；write_file/edit_file 聚合为一张默认折叠的「N 个文件已更改 +X -Y」卡（同一文件一轮多次写入按路径合并为一行，多次写入 ± 取净变化 Σ加−Σ减、单次调用保留真实 hunk 计数，mergeChangedArtifactsByPath/dedupePresentedArtifacts 卡片层去重）（header role=button，点击/Enter/Space 切换 chevron 展开；展开态持久在宿主消息元素 dataset，跨 Lit 重建保留），展开后每行 = Material 文件图标（与文件树同源）+ 文件名/路径同行 + 该轮 +N/-N 统计 + 「审查」+「打开▾」。样式定稿（design-mockups/assistant-reply-file-card-v2.html）：卡片 `width:100%` 填满消息容器与对话正文同宽；± 统计红绿着色与应用 diff token 同源（light green-700/red-700、dark green-300/red-300），折叠头统计组（margin-left:auto）与行内统计均配 GitHub 风格 5px 比例条（段宽按 +N/−N flex 占比，`createDiffBar`）；打开/审查为描边档按钮（1px var(--border) 边框 + background 底，hover 浮 muted 底；撤销保持 ghost 无边框、hover 转红；按钮与行内 ± 统一 0.75rem 字号档）；明细展开用 grid-template-rows 0fr→1fr 动画（visibility 延迟切换防折叠区 Tab 聚焦，reduced-motion 关闭）。「打开｜▾」为分裂按钮：主区点击直接预览（App `onOpenFilePreview` → `openArtifactPreview` 统一入口），箭头区弹菜单（菜单项：预览打开 / 在文件管理器中显示 / 在 VS Code 中打开 / 在 IntelliJ IDEA 中打开，均带图标（品牌图标复用 ProjectOpenMenu 同款资源）；外部打开经 App `onRevealFile(path, target)` → `openWorkspaceExternal(…, target)` 复用既有 open-external 路由（服务端零改动），失败提示按目标区分，选择即执行）；无预览能力（全局会话无项目上下文）时整颗退化为纯菜单形态；菜单模块级互斥（`closeActiveOpenMenu`，removeArtifactCards 时统一清理监听）；菜单（`floating-position.ts` 的 `positionFixedDropdown`）用 fixed 视口定位——右对齐触发器、clamp 进视口、下方不足向上翻（弹层 `-up` 箭头变体）、scroll/resize 即关——逃逸明细动画层 overflow:hidden 与消息列表 overflow-y-auto 的裁剪。行内「审查」经 App `onReviewFileChanges(path)` → `requestWorkspaceInspector({ kind:'review', view:'changes', path })`，review 请求协议新增可选 `path`（`workspace-types.ts`），WorkspaceInspector 请求 effect 直达 `openDiffTab` 单文件 Monaco diff（git 工作区口径，与 ± 统计的会话影子备份口径不同，源码注释已标注）。外层文件卡头部「撤销」按钮区为轮级单一口径：仅「撤销本轮」（`assistantArtifactRollbackTurn`）——该轮 `turnIds.length > 0`（该轮全部产物 toolResult `details.turnId` 的去重保序集合，含重试 run 的 id）且未撤销时渲染，点击经 App `rollbackTurnFromArtifactCard(turnIds)` 打开 `chat/FileRollbackDialog.tsx` 同文件的 `TurnRollbackDialog`（轮级模式，`file-rollback-state.ts` 的 `TurnRollbackState` / `createTurnRollbackController` 驱动，安全分组/反馈/重检复用现有弹窗 UI，无单文件撤销入口）；无 turnId 的轮不渲染撤销按钮。会话级「撤销全部」入口已移除，轮级 `rolledBack` 判定回归纯 `rolledBackTurns`（轮键 = `turnRollbackKey(turnIds)` = `join('|')`）。弹窗 GET `/rollback-turn/preview?turnIds=...` 预检后分组展示安全/不安全文件及 `modified-after-turn` / `external-change` / `stale-backup` 原因；确认经 `rollbackTurn(turnIds, revision)` 提交整轮，200 `completed` / `partial`（两者均按轮标记已撤销，`partial` 冲突文件留在反馈里）、409 `blocked`、500 `failed` 的实际恢复/删除计数留在弹窗展示；网络/超时等展示“结果未确认”并禁止再执行（可重新预检但不自动执行）；App 绑定 agent/session/project，切换上下文关闭弹窗，controller 用 abort + generation 忽略迟到响应（取消请求不保证取消服务端写入）。原生 modal 管理焦点与背景不可交互，初始聚焦标题而非危险按钮；readOnly 面板不传撤销/显示回调。幂等策略为签名跳过重建：每张卡带宿主级 `dataset.quickforgeArtifactSignature`（按宿主消息 + 撤销态 + turnIds 进签名：`rolledBackTurns` 变化 → 该轮卡重建为「已撤销」禁用态；重试在同轮追加新 turnId → 签名变化 → 卡重建让按钮闭包拿到完整 turnId 集合），decorate 周期计划签名一致时仅校正插入位置、不 remove+recreate——下拉菜单/展开态因此能在任意 DOM mutation 触发的重装饰中存活；签名变化才移除旧卡重建并先 `closeActiveOpenMenu`，`removeOrphanArtifactCards` 只删孤儿卡（宿主消息已消失的卡）。卡片口径为每轮一张：`extractTurnArtifacts`（`src/lib/tool-artifacts.ts`）按 user / user-with-attachments 双边界切轮（首条 user 之前的内容归入 `userIndex=-1` 前置组，无轮级撤销），每轮只显示该轮新增产物、挂该轮最后一条 assistant 底部，无产物轮不挂卡；流式 gate 保留（轮结束出卡，流式中不装饰）；提取必须跑在完整 messages（ChatPanelHost 以 `getArtifactMessages` 显式传全量，产物来自 toolResult.details，displayEntries 过滤掉它们）；展开态按宿主消息元素 dataset 保留，不随对话尾部迁移。折叠头总 ± 统计在该轮净变化为 0（该轮加减相抵，文件已被 commit/revert/撤销）时不再渲染 +0 -0 统计与 diffbar，空统计组 div 仍 append 以维持 margin-left:auto 右推布局。
- `decorateEditor` 仅保留 Composer/editor 编排：占位符、只读清理、model selector 开关、left/right controls 定位，以及调用各 focused helper
- 细分实现位于 `panel-decoration/` 子目录：`message-actions.ts`（634 行；复制/回滚/重试/分叉；回合终态错误一行式轻量行——会话最后一条消息为 `stopReason:'error'` 的 assistant 错误时，`turn-error-row.ts` 的 `decorateTurnErrorRow` 把 pi-web-ui 红块（`div.bg-destructive/10` + `<strong>Error:</strong>` + 原文）原地改写为 `⚠ 生成失败 · 译文或原文 [重试] [详情]` 轻量行（与「已停止/重新连接中/模型重试中」同族，红色只保留在图标与该行文字，设计稿 design-mockups/conversation-error-retry.html）：「重试」常显不依赖 hover，统一语义为发送失败 stash → `retryFailedPrompt` 原样重发、否则 `onRetryAfterError` 的 fallbackRetry 走 `retryFromMessage`：若被重试的回合已产生 `toolResult`，则保留整段历史并在末尾追加一条「继续」用户消息续跑（`hasToolResultsAfter` 判定，避免模型丢失已完成的工具副作用而重复改文件），否则仍裁剪最后用户消息之后的内容原地重生成（替代旧 icon-only「继续生成」），点击后行内立即转「⟳ 正在重试…」过渡；「详情」就地展开 mono 原文（仅 `translateErrorMessage` 命中译文≠原文时有按钮，未匹配错误行内即原文）；连续重试仍失败由 `turn-error-state.ts` 的 per-panel tracker（随 agent 重建重置、revision 重建不丢计数）追加琥珀升级行「已重试 n 次仍失败」+ 内联「切换模型」（onSwitchModel → onModelSelect）；门控沿用 `allowRetry`（capabilities.retry）、readOnly、`historyActionsDisabled`（Side Chat 禁用）与流式禁用，历史错误只改写呈现不挂按钮；行内容按呈现签名幂等重建（语言切换/新错误触发重建并自动收起详情），数据层 errorMessage 保持英文原文不动；手动停止状态行本地化——`decorateAssistantStoppedText` 对 `stopReason:'aborted'` 的 assistant 消息把 pi-web-ui 渲染的红色斜体英文 "Request aborted" span 改写为灰色「已停止」（i18n `messageStoppedLabel`，样式类 `quickforge-message-stopped-label`：muted 色、补 px-4 缩进与 6px 上间距，字号沿用 text-sm 与正文同号；设计稿 design-mockups/assistant-stopped-message-preview.html 方案①）；发现路径限定渲染根 div 直接子级 span（`parentElement.parentElement === 消息元素`），避免误伤 tool 卡内同款 aborted 标签（工具级状态由 process-folding 的 `processAborted` 负责）；dataset 记录当前文案幂等、语言切换即时更新，pi-web-ui 与数据层不动，红色（destructive）语义保留给错误；配套地，用户主动停止的回合不再出现失败痕迹——服务端 agent_end 对 signal 已 abort 的运行不再追加 "Request was aborted" 合成错误消息并清 `state.errorMessage`（详见 server README agent-manager 条目），前端 `decorateMessages` 对尾部 assistant `stopReason='aborted'` 的回合隐藏最后一条 user 消息的重试按钮（`trailingTurnAborted` 门控，回滚/复制不受影响；错误终态的一行式错误行重试入口保持）；另在装饰纯文本用户消息时调用 `decorateUserMessageInputClamp`——给 `.user-message-container` 挂 `quickforge-input-clamp` 收起结构：长内容约 6 行定高、底部渐隐 + 居中「展开/收起」按钮，`user-with-attachments` 不参与，实现见 `src/lib/input-clamp.ts`；以及 `decorateUserSlashInvocationChip`——用户消息 `/skill <name>` / `/agent <name>` 前缀渲染为行内 chip（复用 slash-invocation-chip 的共享 chip 元素，消息流内 0.8rem 微调），首文本节点剥前缀、chip dataset 记录被剥字符实现幂等还原，复制仍走 draftTextFromUserMessage 原文不受影响）、`decorateUserContextChips`——只从用户消息 `details.selectedCapabilities` / `details.contextReferences` 读取本轮插件与文件，不从正文或 metadata 猜测；复用只读 `createCapabilityChip` / `createFileReferenceChip` 在 `.user-message-container` 顶部渲染同一 `.quickforge-message-context-references` 行，插件始终位于文件前，documents/spreadsheets/presentations 使用专用图标、未知插件回退通用图标，不传 onRemove 因而无 ×；仅插件/仅文件/混合 aria-label 分别复用现有三态文案，`replaceChildren` 保证重复 decorate 幂等，数据清空移除旧 DOM，复制仍只取原正文）、`composer-plus-menu.ts`（附件和插件菜单；插件项选择走 `selectPluginCapability` 产生结构化插件 chip，不再向正文插入 @mention，菜单打开时与 `/`、`@` 建议浮层互斥）、`agent-access-menu.ts`、`plan-mode-controls.ts`、`send-stop-button.ts`、`model-controls.ts`、`editor-bindings.ts`、`code-blocks.ts`、`process-folding.ts`、`context-compaction.ts`、`scroll-to-bottom-button.ts`（对话区上翻较深时居中悬浮于输入框上方的“回到底部”按钮：280px/120px 滞回显隐、未读徽标、点击平滑回底并恢复自动跟随；readOnly 会话无 composer dock 时自动移除）等；`process-folding.ts` 为每个用户回合维护唯一的“执行中/已执行 · 耗时”顶层状态与折叠入口，中间 Markdown、Thinking、工具和 Subagent 均位于该层级；每段中间 Markdown 后的过程片段继续显示内层阶段聚合标题（状态 + 工具调用数、命令数、编辑文件数），运行中和已完成阶段均默认收起并维护独立折叠状态，普通连续工具仍保留更内层的工具摘要，只有最终回答正文留在组外
- Plan 按钮和 Shift+Tab 切换前端 Plan 模式；发送时复用 `/plan <任务>` 的单轮计划逻辑
- Composer 控件 icon-only 紧凑模式：对话区被左右侧栏拖宽挤压变窄（视口宽度不变，`@media` 视口查询覆盖不到）时，`ChatPanelHost.tsx` 用 `ResizeObserver` 监听宿主内容宽度——<640px 给宿主挂 `quickforge-chat-compact` class、≥672px 释放（32px 滞回防拖动抖动）；`index.css` 的 `.quickforge-chat-panel-host.quickforge-chat-compact` 段将 agent-access/模型/Plan 三个带文字控件收缩为 2rem 只留 icon（与移动端 icon-only 形态一致，Side Chat 复用同一宿主同样生效）；移动端窄视口仍走原 `@media (max-width: 768px)` 块
...
- `GitBranchMenu.tsx` 挂载在主对话标题栏的分支 chip、`GitToolsPinnedSummary` 展开面板（桌面从面板旁边旁挂弹出、移动端固定定位）和 `GitCommitPushDialog` 提交推送对话框头部中，提供分支搜索、当前未提交变更摘要、分支切换、创建并检出新分支，以及 Git 图谱入口；组件新增可选 `style` 透传供旁挂调用方传入动态 top/maxHeight。
- `GitToolsPinnedSummary.tsx` 显示在顶部窗口工具栏中，顶部 `List` 按钮在桌面和移动端始终保留。Workspace Inspector 打开时，仅 `>=1024px` 且非 `mobileShell` 的真实右侧栏场景进入 `suspended`：摘要组件不卸载，保留 closed/capsule/panel、拖动位置、Todo 展开与智能体折叠；toolbar root 与 fixed widget 以 `hidden`/`inert`/`aria-hidden` 真正移出布局、输入、Tab/AT。暂停时不安装 outside/Escape，并停止 resize/ResizeObserver/布局解析，只结束 drag 并清 window listeners、capture、body userSelect、drag/responsive rAF，关闭 branch menu 与 pending focus；不取消用户明确触发的 160ms close timer，不用可被 cleanup 取消的 rAF 归一化 closing/mounted，timer 会在隐藏期间自然完成 `closed => unmounted`，组件最终 unmount 时既有 cleanup 仍清 timer。关闭 Inspector 后按当前目标形态复用方向性布局解析，不 open/minimize、不回焦。PanelRight 打开、Git Changes 与智能体入口统一按“未来打开 Inspector 是否具备 suspension/preserve 能力”分支，真实 desktop 保留 panel，`<1024px`/`mobileShell` 则在打开 Inspector 前先关闭摘要；PanelRight 关闭分支不改 summary，Commit/Push 始终先关闭。`<1024px` 和 `mobileShell` 保持原卸载与全屏 overlay 行为。桌面 `md+` 且非 `mobileShell` 为 closed / capsule / panel 三态：关闭时 List 点击直接打开完整面板；浮层存在时 List 点击完全关闭；完整面板标题栏有独立 `Minimize2` 缩小按钮与 `X` 关闭按钮，**只有 Minimize** 会进入 capsule；摘要胶囊主体点击原位展开，主体末端在约 28px 透明圆形视觉槽中显示 `Maximize2`，与 panel 的 `Minimize2` 配对，默认弱化并仅随主体 hover/focus 克制增强；该槽是主体 button 内的 `aria-hidden` 非独立提示，不形成第三个按钮。右端独立 32px `X` 按钮阻止冒泡并完全关闭。桌面 panel/capsule 外点都 stay，Escape 无摘要级动作且不 preventDefault/stopPropagation；mobile/mobileShell 外点/Escape 仍关闭且无 capsule/minimize。branch menu 桌面从面板旁边旁挂弹出，不因 desktop 外点意外改变摘要形态。胶囊只用三类带图标的语义段：`ListTodo + completed/total`、`FileDiff + +N/−N`、单一 `Bot + running/finished`，Git 空数字回退 List；分段数组统一渲染，第一段无分隔、其余使用 `bg-foreground/15`，ARIA 只朗读实际段。panel↔capsule 共用一个 fixed `data-mode` widget，内容叠放交叉淡化，子层 `transform-origin: top right` 消除中心 scale 的视觉上浮；`ResizeObserver` 记录不受 transform 影响的宽高，panel 展开态另用 `scrollHeight` 保存自然高度，动态 max-height 通过 CSS 变量独立传入，外层宽高保持 220ms 克制过渡，closed↔浮层 160ms opacity/scale，拖动与 reduced-motion 时关闭动画。位置以 `App` 主对话 header 的稳定 React ref 做首次 desktop 锚点：`y = ceil(header.bottom) + 10px`、`x = header.right - targetWidth - 12px`，header 不可用时先回退 toolbar root rect，toolbar root 也不可用时再回退 widget rect；不使用 querySelector/Tailwind selector 或固定 titlebar 高度。方向性布局规则在首次定位、panel/capsule 切换、window resize、drag 过程/结束和 Inspector resume 共用：横向维持 12px 安全区；panel 优先保持当前 top，并把当前位置到 viewport bottom 12px 的空间作为 max-height，由现有 flex 内容区 `overflow-y-auto`；只有下方不足具名最小高度 180px 时才向上移到刚好容纳该高度，viewport 本身更矮时使用全部安全区域。目标 panel 的整屏自然高度不会再把 y 向上 clamp。header rect 仍只在 `positionRef.current` 尚不存在的首次分支读取，后续只解析现有位置；组件生命周期内记忆，不持久化。展开内容按 Git 工具 → Todo → 智能体排列实际存在的分组（智能体组内分「运行中」与「已结束 · N」双小节，已结束默认折叠、标题行整行切换）；首组无顶部分割线，后续组使用紧凑浅色分割线。Git 组提供更改统计、分支操作和提交/推送入口，桌面分支菜单从 panel 旁边弹出（默认左侧、左侧空间不足翻右侧），top 对齐分支行、宽度固定 340px、高度受 viewport 约束。`<md` 与 Android/`mobileShell` 完整保留 List + `fixed inset-x-2 top-14` 面板两态及 fixed 分支菜单 top/max-height 契约，不渲染胶囊/缩小按钮、不启用二维拖动、不设置 `touch-action:none`，X/外点/Escape 关闭；小屏仍隐藏“更改”入口。
...
- Side Chat 默认空白消息区，不显示 Hero、项目选择器、指引或模型信息；显示壳、消息/Composer、回答复制和轮次导航均与主聊天复用同一链路，窄 Inspector 仅由同一响应式规则自然适配。`+`、模型、Access、回滚/重试/分叉等主控件仍在原位置、沿用原样式，但使用原生 disabled；Slash、插件、thinking、附件、权限切换、Plan、文件引用、工具审批、终端执行、context usage/compaction、artifacts、notifications 等不会启用。
- 右侧专业工作区检查器入口为 `WorkspaceInspector.tsx`，采用类浏览器顶部 Tab 工作区；桌面主工具栏右上角的右侧栏（PanelRight）按钮不要求当前项目，global/无项目会话也可展开 Inspector，按钮在模型未配置时禁用并在全断点（含移动端）可见；`lg` 以下窄视口打开时 Inspector 复用 `quickforge-workspace-inspector-fullscreen` 以全屏覆盖展示，无拖拽宽度，右上工具栏 PanelRight 仍可点关闭；`lg` 及以上保持原侧栏布局。Inspector 内 `+` 菜单提供 Files / Review / Terminal / Browser / Side Chat 入口。顶部 Tab 栏高度与对话区 header 保持一致（`h-14`，两者底部横线对齐），右侧 `+`/全屏/终端/关闭按钮与右上角浮动工具栏按钮同尺寸（`size="icon"`），Tab 按钮保持 32px 紧凑，横向滚动容器固定为 Tab 按钮高度，并通过 unlayered 自定义类 `.quickforge-inspector-tab-strip` 隐藏原生滚动条（`scrollbar-width:none` + `::-webkit-scrollbar{display:none}`）；必须写成 unlayered，因为 pi-web-ui 用 unlayered `*{scrollbar-width:thin}` 定义全局滚动条，会压过 Tailwind `@layer utilities` 内的同类声明。Tab 溢出时不再因滚动条占位撑高容器导致 Tab 上下跳动，仍可滚轮/触控板横向滚动与拖拽排序。Tab 下拉列表的条目区最多显示 10 行并在超出时独立纵向滚动，同时按视口高度兜底；“关闭其他 / 关闭全部”操作区固定在滚动区外。除 Goal/subagent 运行时 Tab 外的 Tab 列表、可持久化的活动 `activePanelTabId`、Review 的 Overview/Changes 子视图以及 Reader 左侧导航显示状态按 `projectId + sessionId` 写入浏览器 `localStorage`；Inspector 展开/收起状态不再按 `projectId + sessionId` 持久化/恢复（改为页面生命周期内默认收起，仅用户手动或「附着后新 present_files 自动预览」时打开，旧 `quickforge:workspace-inspector-open:v1:` key 不再读写）。新建空白会话先使用不落盘的 deferred runtime scope，默认关闭、空 Tab、显示 Reader 导航；首次发送创建真实 `sessionId` 时 AgentManager 保留同一个 runtime scope，组件 `key` 不变，内存中的 open/tabs/active/Review/导航状态不会因晋升重建，tab 相关状态由现有持久化 effect 写入真实 `projectId + sessionId` key。切换到其他已存在会话或确认创建另一空白 deferred session 时才更换 runtime scope 并重建 Inspector；底层新建动作 reuse、取消或失败不会由 App wrapper 提前滚动 scope。旧项目级 key 不迁移。Inspector 整体宽度继续使用全局 `quickforge_workspaceInspectorWidth_v2`，拖动上限为动态值：视口宽度的 75%、绝对封顶 1200px、下限 340px，窗口 resize 时已存宽度自动收缩回当前上限内（全屏/窄视口覆盖模式除外）；打开 browser/terminal/document/subagent/reader 内容时的自动展开仍固定拉宽到 640px。Reader 导航分栏宽度等纯布局运行时偏好不纳入会话状态。
...
- Review tab 通过 `/api/git/status` 展示 Git 工作区变更，并在同一个 Review/Changes tab 内通过 `/api/git/file-diff` 展示单文件差异；Changes 顶部刷新按钮右侧提供提交入口并打开 `GitCommitPushDialog`，提交入口左侧常驻项目打开方式菜单：未选择变更文件时在资源管理器、VS Code 或 IntelliJ IDEA 中打开项目，选择文件后则在资源管理器中打开其所在目录或在编辑器中直接打开该文件；Changes 列表提供单文件还原、暂存、退回未暂存、在新标签中打开文件，以及底部批量还原/暂存/退回操作，分别调用 `/api/git/restore`、`/api/git/stage`、`/api/git/unstage`、`/api/git/restore-all`、`/api/git/stage-all`、`/api/git/unstage-all`；标题栏 Git 更改入口会聚焦该 Review/Changes 工作区。`/api/git/file-diff` 对无工作区变更的文件返回 404 `File has no working tree changes`（产物卡的「N 个文件已更改」是会话累计口径，文件被 commit/revert/撤销后 Git 工作区已无变更）；前端 `workspace-api.ts` 的 fetchJson/postJson 抛错时附加 HTTP `status`，产物卡「审查」直达的 diff tab（`openDiffTab`）与 Changes 列表内联展开（`toggleReviewDiff`）两条路径都把该 404 转为本地化友好空态（`workspaceFileNoWorkingTreeChanges`）而非红色错误，diff tab 空态提供「查看文件当前内容」按钮（`workspaceOpenCurrentFile`）降级打开 file reader。`getGitFileDiff` 请求带 10s 超时（TimeoutError 中止，与 `getGitStatus` 同款连接池保护：file-diff 服务端每次先跑全量 git status，无超时的挂起请求会钉死同源连接池一个槽位并拖住其他请求）；`openDiffTab` 对已存在的 diff reader：请求在途时仅激活不重复发起，已是终态（diff/error/noChanges 空态）时重置为 loading 重新拉取（经共享 `loadDiffIntoReaderTab`）——超时/失败或产物被删除后的空态都能通过再次点「审查」恢复，不会永久复用旧结果。
- 标题栏 Git 分支 chip 通过 `/api/git/branches`、`/api/git/checkout`、`/api/git/create-branch` 和 `/api/git/log` 提供分支切换、创建并检出新分支和 Git 图谱弹窗；标题栏的 status 刷新走 `getGitStatus(projectId, signal, { force })`（full 口径）——`titleGitStatus` 同时供 `GitToolsPinnedSummary` / `GitCommitPushDialog` 消费，需要 `files[].additions/deletions`；服务端已支持 `light=1`（跳过 numstat 与行数统计，只返回 branch/counts）与独立缓存键，ChatPanelHost 顶部分支徽标探测已接入（缓存键 `projectId::light` 与标题栏 full 请求分离，首屏同项目最多 full+light 两条短连接）；Workspace Inspector 仍聚焦文件浏览、产物预览和 diff review
