...
```bash
npm install -g @shawnstack/quickforge@1.6.1
qf
...
```text
package-offline/shawnstack-quickforge-1.6.1.tgz
```
...
该包由 `v1.6.1` 标签生成，包含 QuickForge 运行时资源，依赖由 npm 安装。
...
The offline release package for `v1.6.1` is:
...
The package was generated from tag `v1.6.1` and includes QuickForge runtime assets and installs dependencies with npm.
