...
- QuickForge 通过统一模型目录显示已配置且可选择的自定义模型；各入口统一显示模型标签，不把 API Key、Token 或请求 Header 暴露给选择器。
- 主聊天、默认模型、Agent Profile（含 AI Fill）、定时任务、ACP 和可操作共享会话使用同一模型目录。新绑定只保存模型引用，执行时由服务端读取当前 Provider 配置，客户端提交的 Base URL 不能覆盖执行地址。
- 新选择列表会排除隐藏模型；隐藏模型不会成为新会话或新任务的默认候选。
