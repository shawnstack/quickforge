...
- `panel-decoration/change-summary-strip.ts` 为会话级代码变更摘要条 controller（服务端 `session-file-backups.mjs` 影子备份驱动）：在 composer dock 之前插入中性色常驻条（锚点与插入幂等逻辑同 unreachable-strip），显示「本会话修改了 N 个文件」与对账真实 `+N/−N`（绿/红语义色），可展开文件列表（每文件相对路径 + ±行数）；可预览文件带「预览」按钮——类型判定复用 `artifactPreviewMode`（html/图片 → Browser、markdown/代码(.txt/.ts 等) → Reader、pdf/docx/excel → Document），点击经 App 的 `onOpenFilePreview` 走产物预览统一入口 `openArtifactPreview` 分流，与产物列表点开同一文件行为完全同源（含同 URL tab 复用与重载；全局会话无项目上下文不显示按钮，与产物预览一致）；「回滚」按钮两步确认（回滚 → 确认回滚？+ 取消）后 POST `/api/agents/:id/rollback-files` 恢复到会话首次修改前状态；流式中禁用回滚；数据刷新由 decorate 周期 `sync()` 检测消息里 write_file/edit_file toolResult 签名变化后节流 fetch `GET /api/agents/:id/file-changes`，0 文件整条移除；Side Chat / readOnly 不挂载（ChatPanelHost 主 effect 创建，随 agent 重建，destroy 时清理）
