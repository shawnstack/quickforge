...
- `agent-subagent-runner.mjs` — `run_subagent` 生命周期：`runSubagent`、临时 profile、超时/中止进度摘要、错误 details 构建、trace 截尾与 `SUBAGENT_*` 常量。实时 trace、终态与错误终态 details 复用同一个 `subagentRuntimeDetails`：`model` 在继承模式下合并实际模型的 `provider/id/name`（保留 `mode/inherited`），并写入已解析的 `thinkingLevel`，供前端运行详情「运行信息」行展示。
