...
- Android 薄壳入口由 `components/mobile/MobileServerConnectPage.tsx` 与 `lib/mobile-server.ts` 提供：仅接受 `.ts.net` MagicDNS 完整域名或 Tailscale `100.64.0.0/10` 地址，保存服务器地址后直接导航到远端 QuickForge 页面，保持页面、REST、SSE 和 LAN Cookie 同源
