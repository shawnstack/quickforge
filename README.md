...
```bash
npm install -g @shawnstack/quickforge@1.6.5
qf
...
```text
package-offline/shawnstack-quickforge-1.6.5.tgz
```
...
该包由 `v1.6.5` 标签生成，包含 QuickForge 运行时资源，依赖由 npm 安装。
...
The offline release package for `v1.6.5` is:
...
The package was generated from tag `v1.6.5` and includes QuickForge runtime assets and installs dependencies with npm.
