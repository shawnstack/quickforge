...
- token 规则为 `(^|\\s)@([^\\s@]*)# `src/components/` — React 组件
...
；裸 `@` 与 1 字符 query 只显示继续输入提示，不提供最近文件；2+ 字符经 300ms debounce 请求 `/api/workspace/mention-search?projectId&query&limit=8`，AbortController + generation 丢弃旧响应；仅接受 `type:'file'` 且安全相对路径。
