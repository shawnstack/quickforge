...
| API Key | 服务商提供的密钥；本地代理或 Ollama 可留空 |
...
| 工具 | 作用 |
|---|---|
| `list_dir` | 列出项目目录中的文件和文件夹 |
| `read_file` | 读取项目内的文本文件 |
| `grep_files` | 搜索项目文件内容 |
| `write_file` | 创建或覆盖文件 |
| `edit_file` | 用精确文本替换编辑文件 |
| `run_command` | 在项目目录中运行命令 |
...
通常情况下，本地 QuickForge 服务已经负责应用存储，不一定需要代理。以下情况可能需要开启 CORS Proxy：

- 浏览器直接调用某些 LLM Provider 时遇到 CORS 限制。
- 使用需要通过代理转发的服务。
- 特定 Provider 要求通过兼容代理访问。
