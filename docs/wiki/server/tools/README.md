...
- **写入防误**: `write_file` 验证文件在项目内；`edit_file` 确保 `oldText` 唯一匹配。二者共享 `session-file-lock.mjs` 全局进程内锁，覆盖读取旧内容、before/pending 日志、实际写入与预期 `afterHash` 提交；会话备份持久化失败阻止工具继续写入，写入中断保留不安全状态。续写校验与上次 after 的连续性，外部改动链不会被后续工具写入“洗白”；Windows 别名验证同一目标后去重，不可信路径拒绝。安全撤销支持每项独立 revision：普通兄弟项冲突/legacy 不影响安全单项，整批仍要求剩余全安全；会话忙、索引异常或失败回滚 intent 全局阻止。独立 `rollback-intent` marker 仅在最终索引提交成功后清除，失败保留 intent 并阻止后续受控写入；成功撤销项从剩余 preview/summary 排除，再写入该项以当次 before 新建记录。带父会话 `sessionId` 的子 Agent 同类工具也纳入备份/撤销；shell 及 MCP 独立写入不在覆盖范围，锁不阻止外部进程，也不扩展 Git index 操作。`write_file`/`edit_file` 的 toolResult `details` 返回 `turnId`（当次所属轮标识，由 toolContext `getTurnId` 供给；无轮次写入为 `null`，如合成路径或旧会话恢复后的写入），备份版本 meta 同步携带 `turnId` + `toolCallId`，供轮级回滚按轮归因。
- **命令超时与长输出**: `run_command` 默认超时 1 小时，支持通过 `timeoutMs` 在安全上下限内调整；运行中和最终结果默认只向模型/界面返回 stdout/stderr 预览：每路最多最后 200 行，且 `stdout_preview + stderr_preview` 合计最多 10,000 字符。若发生行数或字符数截断，结果会设置 `truncated: true`，并同时提供 `stdout_truncated`/`stderr_truncated` 及兼容旧字段 `stdoutTruncated`/`stderrTruncated`/`outputTruncated`。完整 stdout/stderr 会写入 `~/.quickforge/logs/commands/`，结果通过 `outputFile` 指向日志文件。Agent 运行中的 `run_command` 会按 `toolCallId` 登记，前端工具卡片可手动终止。`run_in_background: true` 时调用立即返回（`details.background/running/detached` 与 `taskId`），忽略 `timeoutMs`，进程脱离当前回合并持续把 stdout/stderr 追加到同一日志；进程退出或被终止后，会话收到 `<task-notification>`（含输出文件路径）。当前回合仍在流式时通知走 `followUp`，空闲时直接开始下一轮；活跃 Goal 只暂存通知。`read_file` 可按返回的绝对路径读取该命令日志，其它工作区外路径仍拒绝。会话销毁或服务关闭会终止该会话仍在运行的后台命令。运行列表通过会话状态和 `background_commands` 事件同步；进程退出时会把原 `toolResult` 替换为终态。
