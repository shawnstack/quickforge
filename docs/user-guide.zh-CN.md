...
- 关闭模型的 **在模型选择中显示** 开关，保留配置但从模型选择列表隐藏。
...
- QuickForge 通过统一模型目录显示已配置且可选择的自定义模型和 QuickForge Cloud 模型；各入口统一显示模型标签，不把 API Key、Token 或请求 Header 暴露给选择器。
- 主聊天、默认模型、Agent Profile（含 AI Fill）、定时任务、ACP 和可操作共享会话使用同一模型目录。新绑定只保存模型引用，执行时由服务端读取当前 Provider/Cloud 配置，客户端提交的 Base URL 不能覆盖执行地址。
- 新选择列表会排除隐藏模型和 Cloud 目录中 `available:false` 的模型；隐藏模型不会成为新会话或新任务的默认候选。
- 已经使用隐藏模型的会话、Agent、定时任务或 ACP 会话仍保留当前绑定，编辑时可看到当前值，但切换离开后不能重新选择该隐藏模型。
- 已保存的默认/当前偏好失效时，新会话会回退到第一个当前可选模型；Profile、任务和历史会话等显式绑定失效时会报错，不会静默换模型。
- 可操作共享默认不能使用分享者的 QuickForge Cloud 模型和额度；创建者必须显式开启该权限。只读共享不能执行模型。
...
在 **设置 → QuickForge Cloud** 中：

1. 本地状态点击 **登录或注册**；QuickForge 会在这个显式动作内先创建临时游客身份，再开始登录。
2. 游客状态点击 **升级为正式账户**，会保留当前 installation。
3. 打开云端验证页，按页面提示注册或登录；邮箱和密码只提交给 Cloud 页面，QuickForge 不读取、不保存。
4. QuickForge 会显示设备码、倒计时并自动检查结果；页面刷新或本地服务重启后仍可继续。
5. 取消、拒绝、过期或临时网络失败不会删除原游客 Session；成功后才替换为正式账户，并显示邮箱和套餐。

待处理登录期间不要切换 Cloud URL；pending flow 与发起时的服务 URL 绑定，跨 URL 请求会被安全拒绝。
