...
- **Goal 模式**: 主聊天与定时任务可用 `/goal <目标>` 设定带可验收标准的目标；ACP、channel 与 shared 请求仍拒绝 Goal。流程先整轮只读规划，正常轮末持久化后自动执行（累计时间无限）；定时任务与主聊天均复用既有 `runPrompt`→runner，不新增执行器。模型只能提交绑定真实工具结果的证据；`complete` 经证据检查、本轮正常结束及消息/最终状态持久化成功后自动 `completed`，不需人工 accept；无法自动验证时 `needs_review` 报告落 `blocked` 并说明原因，不要求人工签字。`waitForGoalCompletion` 复用 runner 的持久化终态屏障与 idle，只有 `completed` 才算定时任务成功；`paused`/`blocked`、预算耗尽、awaiting 等仍视为 running 并保留 serial，关联聊天可 resume/cancel，静止后的 cancelled/failed 才使任务失败。Goal 豁免普通 1h/Profile 墙钟超时，但审批、轮次、工具超时不提升或取消；普通任务行为不变。重启沿用 stale-run failed，Goal paused 不自动重放。Profile 获准的 `goal_report` 不受 workspace 工具白名单过滤，但不扩展 workspace 能力；goal waiter 避免 idle eviction。详见 [服务端预算恢复契约](server/README.md#goal-预算追加与恢复) 与 [客户端编排](src/lib/README.md#goal-预算追加客户端契约)
