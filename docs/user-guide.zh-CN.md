...
1. 本地模式点击 **登录或注册**，直接启动 Device Flow，不再创建临时游客。
2. 打开云端验证页，按页面提示注册或登录；邮箱和密码只提交给 Cloud 页面，QuickForge 不读取、不保存。
3. QuickForge 会显示设备码、倒计时并自动检查结果；页面刷新或本地服务重启后仍可继续。
4. 取消、拒绝、过期或临时网络失败会保留本地/未登录状态；成功后显示账户邮箱和套餐。
