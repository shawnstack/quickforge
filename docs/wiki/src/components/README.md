...
- `GitToolsPinnedSummary.tsx` 显示在顶部窗口工具栏中，顶部 `List` 按钮在桌面和移动端始终保留。Workspace Inspector 打开时，仅 `>=1024px` 且非 `mobileShell` 的真实右侧栏场景进入 `suspended`：摘要组件不卸载，保留 closed/capsule/panel、拖动位置、Todo 展开与智能体折叠；toolbar root 与 fixed widget 以 `hidden`/`inert`/`aria-hidden` 真正移出布局、输入、Tab/AT。暂停时停止 outside/Escape/resize/ResizeObserver，结束 drag 并清 window listeners、capture、body userSelect、rAF、closing/focus 工作，只关闭 branch menu；关闭 Inspector 后按当前目标形态重新 clamp 到 12px 安全区，不 open/minimize、不回焦。PanelRight click marker 防止 document pointerdown 在 Inspector state 生效前把 panel 预先缩成 capsule；Git Changes/智能体打开 Inspector 不再永久清 `expanded`，Commit/Push 仍先关闭。`<1024px` 和 `mobileShell` 保持原卸载与全屏 overlay 行为。桌面 `md+` 且非 `mobileShell` 为 closed / capsule / panel 三态：关闭时 List 点击直接打开完整面板；浮层存在时 List 点击完全关闭；完整面板标题栏有独立 `Minimize2` 缩小按钮与 `X` 关闭按钮；摘要胶囊主体点击原位展开，右端独立 32px `X` 按钮阻止冒泡并完全关闭。桌面 panel 点击外部收起为 capsule，capsule 点击外部保持常驻；Escape 完全关闭并回焦顶部 List。胶囊只用三类带图标的语义段：`ListTodo + completed/total`、`FileDiff + +N/−N`、单一 `Bot + running/finished`，Git 空数字回退 List；分段数组统一渲染，第一段无分隔、其余使用 `bg-foreground/15`，ARIA 只朗读实际段。panel↔capsule 共用一个 fixed `data-mode` widget，内容叠放交叉淡化，`ResizeObserver` 写入实测尺寸，外层宽高 220ms 克制过渡；closed↔浮层 160ms opacity/scale，拖动与 reduced-motion 时关闭动画。位置以原 toolbar root rect 首次对齐右上入口，随后按 viewport 约 12px 安全边距 clamp，resize 与形态变化会重算；仅组件生命周期内记忆，不持久化。展开内容按 Git 工具 → Todo → 智能体排列实际存在的分组（智能体组内分「运行中」与「已结束 · N」双小节，已结束默认折叠、标题行整行切换）；首组无顶部分割线，后续组使用紧凑浅色分割线。Git 组提供更改统计、分支操作和提交/推送入口，桌面分支菜单按左右可用空间动态选择方向。`<md` 与 Android/`mobileShell` 完整保留 List + `fixed inset-x-2 top-14` 面板两态及 fixed 分支菜单 top/max-height 契约，不渲染胶囊/缩小按钮、不启用二维拖动、不设置 `touch-action:none`，X/外点/Escape 关闭；小屏仍隐藏“更改”入口。
