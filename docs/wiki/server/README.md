...
- 支持 LAN 共享（显示局域网 URL）；远程完整访问需在本机配置密码。已认证远端可使用 Storage、Backup、更新与重启，不再依据客户端 IP 网段区分；终端、系统代理、终端 Shell、目录选择器和打开服务端电脑上的资源管理器/IDE 仍仅限本机。
- 启动后初始化 `network-proxy.mjs`：读取 `settings['network-proxy']`，为外部 Fetch 请求应用直连、操作系统真实代理、手动 HTTP(S) 代理或 PAC 地址；localhost 始终直连。Desktop 默认 fork 模式与 CLI/SDK 一致由 `@vscode/os-proxy-resolver` 调用 Windows、macOS 和 Linux 的原生系统代理来源，当前不支持自定义 PAC 地址，且不会静默降级为直连；仅 `QUICKFORGE_DESKTOP_INLINE=1` 显式 inline 时由 Electron/Chromium Session 处理系统 PAC/WPAD 和自定义 PAC 地址
...
- `agent-subagent-runner.mjs` — `run_subagent` 生命周期：`runSubagent`、临时 profile、超时/中止进度摘要、**部分成果回传（partial work report）**、错误 details 构建、trace 截尾与 `SUBAGENT_*` 常量。部分成果回传：成功/运行期失败/超时/父运行中止四路径都把「Work done 报告」（工具调用清单 + 全量 assistant 正文）追加到 toolResult 正文回传父模型，父模型可据此了解 subagent 内部行为、续接部分成果或只重派剩余工作；不做条数/行数限制，唯一截断是工具参数单行摘要（`SUBAGENT_TOOL_ARGS_SUMMARY_LIMIT` 200 字符）；成功时最终回复置顶并按索引跳过报告中与之重复的那条正文；超时/中止错误首句（含进度摘要）逐字保留为前缀、通用失败首行保持上游原文（前端 subagent trace 去重以 trace 终态错误文本精确比较，快照先于追加、自洽不受影响），报告经 toolResult output 块在运行详情展示；全量 messages 仍只进 `details`（`omitDetailsForLlm` 不送 LLM），报告是父模型了解过程的唯一通道。实时 trace、终态与错误终态 details 复用同一个 `subagentRuntimeDetails`：`model` 在继承模式下合并实际模型的 `provider/id/name`（保留 `mode/inherited`），并写入已解析的 `thinkingLevel`，供前端运行详情「运行信息」行展示。
...
- `session-file-backups.mjs` — 会话级影子备份位于 `~/.quickforge/cache/global/session-backups/<sessionId>/`：`write_file`/`edit_file` 首次写入保存 before 内容/hash（新建文件登记 created），先持久化 pending 与预期写入 hash，写盘后提交 `afterHash`；每次续写校验旧内容与上次 after 的连续性，已发现的外部改动、不完整写入或旧备份不会因后续写入被重新标为安全。缺少可信 after/路径元数据的旧备份拒绝撤销；Windows 路径别名须验证同一文件后合并，重复索引、不可验证别名及不可信路径（含符号链接/硬链接、工作区根变更）拒绝处理。`getSessionFileChanges` 保持原摘要结构，但不读取不可信目标。备份 entry 已版本化（per-turn-artifact-cards）：每次受控写盘都在 `versions[]` 追加一条独立 blob 快照 `{turnId, toolCallId, created, beforeHash, blobName, beforeBytes, afterBytes, afterHash, timestamp}`（新建文件首版本复用 entry 级 blob），entry 级 `beforeHash`/`backupName` 恒锚定首版本，会话级/单文件回滚行为不变；旧格式 entry 视为隐式单版本（`turnId:null`），轮级查询不可见。
  ...
  - 回滚先持久化独立 `rollback-intent` marker，再提交索引与逐项执行状态；marker 仅在最终索引提交成功后清除，失败路径保留已持久化 intent 并启用不可用保护，避免重启或旧索引把失败误认为可重试。成功项标记 `completed`，从剩余 preview/summary 排除；尚有剩余项返回 `partial`，全部完成才返回 `completed`。再次写入已完成项以当次 before 新建记录，整批完成后的新写入另起批次；物理 blob 留待 7 天 TTL 清理（新备份时触发、至少间隔 1h）。仅覆盖带会话上下文的 QuickForge `write_file`/`edit_file`，子 Agent 同类工具归因父会话；不涵盖 shell 或 MCP 独立写入，也不操作 Git index。
  - 轮级回滚（per-turn-artifact-cards）：`getSessionTurnRollbackPreview` / `rollbackSessionTurn` 按 `turnIds` 集合（一轮 = 原 run + 重试 run 的全部 turnId）只读预检与执行，复用全局文件锁、pending `rollback-intent` marker、双 preflight（整批 revision + 逐项 fresh 复查）、失败即停不补偿与 blob TTL 清理等既有纪律；只执行安全子集，不安全文件按文件返回 `conflicts:[{path,reason}]` 而非整体阻断（存在冲突时返回 `partial`）。版本按集合匹配（`version.turnId ∈ turnIds`），安全判定 reason 枚举：`modified-after-turn`（集合首匹配版本之后该文件还有 turnId ∉ 集合的版本写入，含无归属写入——此时恢复会静默撤销集合外轮次的工作）、`external-change`（磁盘 hash ≠ 该轮最后版本 afterHash，含外部删除）、`stale-backup`（版本记录不完整 / blob 校验失败 / 路径校验失败）；`restore` = 写回集合首匹配版本 before，`delete` = 删除该轮新建文件。集合内无任何记录返回空 files（合法答案，前端据此识别不可撤销轮）。
- 拆分期间的内部共享导出（`resetIdleTimer` / `createServerTools`，登记于契约测试 `INTERNAL_SHARED_EXPORTS` 清单）后续随对应块迁移后收回。


**功能**:
- Agent 创建（`createAgent`）：初始化 QuickForge Agent，按传入配置解析模型、权限上下文、system prompt 与消息历史；已有内存会话复用并仅更新 idle retention。
- Provider 请求重试与流超时：主 Agent、Subagent、对话压缩和辅助模型生成默认设置 `maxRetries: 3`，即首次请求失败后最多再重试 3 次（最多共 4 次请求）；是否可重试及退避由各 Provider 实现决定，模型连通性探测显式保持不重试，`maxRetryDelayMs` 只限制服务端要求的单次等待上限。统一 AI 流包装默认等待首个实质事件 90 秒；首个实质事件（任意非 `start` 流事件，包括文本、thinking、tool call）到达后，空闲超时切换为 3 分钟，另有 20 分钟总时长硬上限；同步等待模型完成的 4 个路由接口（模型连通性探测 60 秒、Agent Profile AI 填充 3 分钟、定时任务自然语言解析与 Git 提交信息生成各 2 分钟）按场景覆写更紧的 total 预算，常量位于 `ai-provider-options.mjs`。仅首个实质事件前的 idle 超时可透明重建上游流，最多重试 2 次；已有实质内容、用户中止或总时长超时均直接失败且不重建。显式 `idleTimeoutMs` / 兼容 `deadlineMs` 仍同时覆盖首事件与后续 idle 两档，但不改变“已有内容后不重试”的规则；超时会中止 Provider signal。
- Git 提交信息 AI 生成同样接收 `modelRef` 并通过统一 resolver；客户端提交的完整模型仅作兼容识别，不能覆盖 Provider Base URL。
...
- 模型配置即时刷新（`refreshAllSessionModels`）：`custom-providers` 存储（模型定义、Max Tokens 等）经 storage 路由（PUT/DELETE key、DELETE 整 store）或备份恢复变更后，遍历内存活跃会话重新解析 model 绑定（跳过 streaming 中会话——由下一次 `runPrompt` 的 `refreshSessionModelBinding` 刷新），仅当 `session.model` 实际变化时 emit `state` 事件推送前端；模型被删除导致的解析失败只记日志并保留最后绑定，由下一条消息复现原报错。前端 `ServerAgent` 复用既有 `case 'state'` 更新 `state.model`，`useAgentManager` 同步 `activeModelRef` 并 bump `chatPanelRevision` 触发重渲染。
- Subagent 工具：`run_subagent` 在父会话内创建短生命周期临时 Agent；运行条件是父会话已解析出有效 `projectContext.workspaceRoot`，因此项目对话和合成默认 workspace 的全局对话都可使用，不再要求必须存在真实 `projectId`。可调用启用的 Agent Profile。内置 `explore` 是只读仓库调研的首选，用于文件发现、源码搜索、调用链追踪、测试/文档/wiki 发现和影响面分析，可执行安全的检查/诊断命令但不能修改文件；内置 `general` 适合有边界的复杂多步骤实现或更广泛独立任务，可使用完整内置工作区工具但不含 MCP/Skills。自定义 Agent Profile 也可通过白名单工具执行。`run_subagent` 还支持 AI 按需传入一次性 `temporary` profile spec；服务端会校验名称、工具、`capabilityPolicy` 和模型引用，将该临时 subagent 写入 `~/.quickforge/cache/global/tmp/agents/<session>/<run>/*.md` 后再执行，并在结果 details 中返回 `profilePath`、`source`、`lifecycle`、`capabilityPolicy` 和实际模型信息。子 Agent 不作为普通会话持久化，默认不能递归调用 `run_subagent`。父会话会在内存中保留正在执行的工具快照，`getSessionState()` / SSE 初始 state 会将尚未进入权威消息历史的 `run_subagent` partial trace 和 `pendingToolCalls` 返回给刷新后的页面；运行期 trace 更新（150ms 节流）的 `details.messages` 只携带最近 50 条（`SUBAGENT_TRACE_MESSAGES_LIMIT`）并附 `messagesTotal` 总条数，终态 `toolResult` 的 `details.messages` 保持全量；该快照不写入持久化会话，也不进入 LLM 上下文，最终权威 `toolResult` 出现后自动去重清理。执行链将 `run_subagent` 的 `toolCallId` 透传至 Subagent runtime；profile/task/workspace/model 等前置校验通过后立即生成子 session ID 并记录 `started`，后续模型解析、工具创建、system prompt、Agent 构造或 prompt 任一失败均由终态守卫只记录一个 `failed`，同时保留 `timeout_triggered`、`parent_aborted`、`settled_after_abort`、`completed` 既有语义和不重复终态日志。字段仅包含 timeout/duration/toolCalls、abort 后等待与 outcome 等元数据，不记录 task/context/messages/system prompt/工具正文/profilePath/完整错误正文。Subagent 内部 AI stream retry/timeout 使用仅内部白名单日志上下文关联同一组 ID，该字段在进入 Provider 前删除。Subagent 运行时长默认与硬上限均为 2 小时（`SUBAGENT_DEFAULT_TIMEOUT_MS` / `SUBAGENT_MAX_TIMEOUT_MS`；内置 explore/general、自定义与文件 profile 的默认 `max-runtime-ms` 同步上调，旧安装的内置 markdown 由启动物化按全内容比对自动重写）。超时错误正文在既有首句后追加进度摘要（工具调用数、被中断时仍在执行的工具、最后一条 assistant 文本压缩空白并截断 600 字符），使父模型在 `details` 不进入 LLM 上下文的前提下也能了解部分成果；同时父 Agent 的 `afterToolCall` 会把抛错前暂存的 `quickforgeSubagentDetails`（与成功终态同构：`timedOut:true`、全量 `messages`、`toolCalls`、`pendingToolCalls` 等，按 toolCallId 暂存并即取即删，遗留条目按 TTL 兜底清理）注入错误 `toolResult.details` 持久化，刷新/恢复会话后 Inspector 仍可见完整过程，前端按 `details.timedOut` 判定错误态；父运行中止（`aborted with parent run`）复用同一机制：正文追加 `Progress before abort:` 同构摘要，details 以 `aborted:true` 标记注入（前端 `details.aborted → error` 判定同样为既有逻辑）；其余运行期失败（如 subagent 内部模型流错误）由内层 catch 统一附带同构 details（不带终止标记、错误正文保持上游原文以维持前端 stripTerminalErrorFromTrace 的去重匹配），因此任何错误 toolResult details 均携带可恢复的过程信息
- Agent Profile 执行：`createAgent` 支持传入 `agentProfile`，在默认系统提示词后追加 profile 系统提示词，并按 `allowedTools` 限制 workspace 工具；定时任务可绑定 profile 执行。自定义 Profile 可将模型和思考等级设为继承或固定值；运行时先解析最终模型，再解析思考等级，非推理模型统一降级为 `off`。内置 Profile 的定义保持只读，模型、思考等级和 `allowMcpTools` / `allowAgentSkills` 可通过 `agent-profile-overrides` 覆盖，关闭项会清除对应覆盖字段。
...
- **可用范围**：QuickForge 主聊天与定时任务可用；ACP 会话、渠道会话（持久化 `source`）与共享会话（请求级 `source:'shared'`）仍拒绝。请求级来源只拒绝该次请求，不会永久关掉 owner 的 goal 能力。定时任务复用既有 `runPrompt`→Goal runner，不新增执行器。
- **流程**：`/goal` → 只读规划 → plan 内部 awaiting_confirmation 过渡 → 正常轮末与持久化屏障 → 自动 running → complete 可信证据检查及正常轮末持久化 → completed。定时任务通过 `waitForGoalCompletion` 复用 runner 的持久化终态屏障与 idle；仅 `completed` 才 schedule 成功，`paused`/`blocked`、预算耗尽与 awaiting 等继续视为 running 并保留 serial。关联聊天可 resume/cancel；静止后的 cancelled/failed 才使定时任务失败。当前 needs_review 报告落 blocked 并说明无法验证原因，不伪造 passed，不要求人工签字；历史 human evidence 与旧 accept API 保留。
- **预算与超时**：默认 20 轮（settings 键 `goal-settings` 可配置，设置·常规页「Goal 最大轮次」数值项写入，整数 clamp 1–100，读取失败回落 20；新建 goal 预算与追加额度同源），无累计时间限制（maxActiveDurationMs: null）；Goal 豁免普通 1h/Profile 墙钟超时，新 Goal 不创建 duration watchdog，累计用时继续记录。审批、轮次、工具超时不提升或取消；不使用 Infinity 或巨型 timer。恢复历史保持旧数值，显式 resume/extend_resume/revise 才移除旧时间上限。普通任务超时语义不变。
- **生命周期纪律**：每轮结束只有在运行真正结束且最终状态持久化成功后才调度下一轮（fail-closed：持久化失败则暂停为 `paused`/`blocker:'persist_failed'`）；`goalRun` 结算窗口用显式 barrier（`activePromptPromise`）与 abort generation 观测用户中止，避免 aborted 后仍继续。goal body 随会话 CAS 权威快照持久化，metadata 只存 `{id,status,updatedAt}` 投影；重启沿用 stale-run failed，Goal paused 不自动重放任何轮次。goal waiter 持有 idle eviction 保护，避免等待期间 Agent 被驱逐。
...
- **与其他机制的关系**：规划轮只读（无写/命令/MCP/插件，子 Agent 仅只读）；报告 `complete` 后本轮剩余只允许只读检查；needs_review 转 blocked 后只允许 read_file/grep_files 与必要 ask_user，本轮不允许更多写入、命令、委派或 goal_report，问答或错误结束均不自动续跑；Profile 获准的 `goal_report` 不受 workspace 工具白名单过滤，但不扩展 workspace 能力。活跃 goal 期间普通 prompt/retry/steer/follow-up/消息回滚均被拒（`GOAL_ACTIVE`），终止后随下一条普通消息清除卡片；审批/ask/用户停止会驱动 goal 状态（`awaiting_approval`/`awaiting_input`/`paused`）。Goal 模式不会自动执行文件撤销或 git 提交/发布，这些仍由用户显式操作。
...
- `session/new` / `session/load` 会返回 ACP `configOptions` 模型和 Thinking Level 下拉选项，模型来源于统一 Model Catalog，包含当前允许的自定义模型；当前 ACP 会话已经使用的隐藏模型会保留在该会话的选项中，但不能由其他会话重新选择。客户端调用 `session/set_config_option` 后会通过统一 resolver 校验并切换当前 ACP 会话配置。切换到不支持 reasoning 的模型时会自动将 Thinking Level 置为 `off`。新建会话时初始 Thinking Level 与 Web UI 保持一致：优先读取用户在设置中保存的默认思考级别（`settings['default-options'].thinkingLevel`），否则推理模型默认 `high`、非推理模型默认 `off`（见 `resolveInitialThinkingLevel`）。
...
- `sqlite/session-state-worker-protocol.mjs` / `session-state-worker.mjs` / `session-state-worker-client.mjs`（2026-09-07 新增）把会话域**重操作**搬进 worker_threads 专职线程：白名单 op（`save/saveMany/applyBatch/replaceMessages/appendMessages/delete/deleteBySessionId/replaceAll/exportSnapshot/verifyIntegrity/checkpointWal/readMessagesPage`）经 `postMessage` RPC 在 worker 侧自开的 DatabaseSync 连接上执行，消息编码与同步大事务不再阻塞主事件循环（所有运行时统一受益：desktop fork/inline、`qf` CLI/npm web、ACP stdio）。双连接分区：主线程保留全部小读（get/findBySessionId/messageCount/readLastMessage 等）与 share/lan/scheduled-runs/session-index/maintenance-lock；正确性靠既有 revision CAS + `BEGIN IMMEDIATE` + busy_timeout（worker 侧另有 SQLITE_BUSY 有限重试），postMessage 天然 FIFO 保持 per-session 串行。错误序列化保留 `statusCode/errorCode/expectedRevision` 控制流属性（CAS 冲突重试在主线程不变）；worker 崩溃时在飞请求以 `SESSION_STATE_WORKER_CRASHED` 拒绝、下次调用自动重建（懒启动、按数据库路径键控，close/reopen 循环透明重连）；`closeSqliteStorage` 经 `registerSqliteStorageCloseHook` 先关 worker 连接再关主连接。环境变量 `QUICKFORGE_SQLITE_WORKER=0` 为 kill-switch（回退主线程直连同步路径）；`configureSessionStateService({ repository })` 注入的测试/维护 repository 恒优先（heavyOp 直连注入对象，不经 worker）。
...
- `session-state-service.mjs`（存储 v2）恒走 SQLite repository（无 phase 路由、无 mirror drain）；`messageStoragePlan` 决定增量写计划（body-only / replace / append：长度对齐时尾 digest + 中部采样校验，防中部原位编辑丢失）；`saveSessionStatePair` 为 async 且带 `expectedRevision`（CAS 保护）时走 `savePairChunked`：入口对 messages 浅拷贝冻结调用时快照（防让出间隙调用方 push 进后续编码批次），先分批让出式编码（`encodeMessagesChunked`，仍在主线程、保快照契约）再把预编码行交 `heavyOp` 在 worker 线程跑事务，yield 间隙的并发写入由 revision CAS 转为 conflict 重试；`synchronize` 只深拷贝 body、messages 仅浅拷贝（写入字节与全量深拷贝路径一致）；无 CAS 的调用方同样经 `heavyOp`（worker 或 kill-switch 直连）。重操作方法（`saveSessionBody/saveSessionMetadata/deleteSessionState/updateSessionMetadataBucket/replaceSessionStateStore/applySessionBatch/exportSessionStateSnapshot/replaceSessionStateSnapshot/getSessionStateDiagnostics/readSessionStateStore/readSessionStateValue` 及内部 `savePair*`/`assembleState`）均为 async 并路由 worker（见上条）；小读（`readSessionStateRecord/storedMessagesState/readSessionKeys/hasSession/readSessionIdentityRows/readSessionMetadataBuckets/readSessionStorageState`）保持同步主线程。异步化打开的 pin-vs-persist 交错窗口由两侧 CAS 有界重试兜底（`atomicSessionMetadataStateUpdate` 重试须 `return await` 才能捕获异步 conflict；storage.mjs 的 `atomicSessionMetadataBucketUpdateViaFacade` 按 service 同款 maxRetries=3 用新鲜桶状态重算 updateFn）。返回 `messageStoragePlan`/`messageCount`，`storedMessagesState`（行数 + 尾部行 digest）供 agent-manager 冲突检测；`readSessionMetadataBuckets` 只读 `meta_json` 投影（不物化正文与消息行）；`readSessionStorageState` 返回常量 `authoritative` + 实时会话数。启动链见下条 import。
...
- 校验 Agent 名称、系统提示词、工具白名单、`capabilityPolicy`、模型引用、运行时间和工具调用预算；Markdown/API 的 `allowMcpTools` 与 `allowAgentSkills` 缺失时默认关闭。
- 为 `run_subagent`、定时任务和前端 Agents 页面提供统一列表；QuickForge 自定义 Profile 可编辑两个能力开关，外部 readonly Profile 不可编辑，内置 `general` / `explore` 仅通过 `agent-profile-overrides` 编辑这两个开关、模型和思考等级。子 Agent 只继承父会话当前实际工具集中对应的 MCP/Skills 工具，不从全局额外扩展，MCP 保持父会话审批语义，关闭 Skills 时不注入可用 Skills 目录，并禁止递归 `run_subagent`。
