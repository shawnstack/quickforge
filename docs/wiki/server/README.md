...
- 工具管理：基于 Skills 和 YOLO 模式动态构建工具列表；`/plan` 当前轮会额外阻止写文件、编辑文件、运行命令和 subagent 执行，只允许生成计划；`/review` 当前轮允许读取和运行检查命令，但阻止编辑文件和 subagent 执行，用于提交前自检。
...
- `handleInternalCommand()` — 处理内置命令，包括 `/plan`（只生成计划，本轮禁止写入/命令执行）、`/review`（提交前自检，本轮禁止编辑文件）、`/compact`、`/clear` 等
