...
export function GoalSummarySection({ goal, sessionId }: GoalSummarySectionProps) {
  ...
  return (
    <button type="button" className="quickforge-goal-summary-entry" onClick={() => requestOpenGoalSummary(sessionId, goal.id, 'progress')} aria-label={t('goalOpenSummary')}>
      <span className="flex min-w-0 items-center gap-2"><Target className="size-3.5 shrink-0" /><span className="shrink-0 text-muted-foreground">{t(view.statusKey as AppTextKey)}</span><span className="truncate">{goal.objective || t('goalObjectiveEmpty')}</span></span>
