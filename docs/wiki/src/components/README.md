...
- Composer 控件 icon-only 紧凑模式：对话区被左右侧栏拖宽挤压变窄（视口宽度不变，`@media` 视口查询覆盖不到）时，`ChatPanelHost.tsx` 用 `ResizeObserver` 监听宿主内容宽度——<640px 给宿主挂 `quickforge-chat-compact` class、≥672px 释放（32px 滞回防拖动抖动）；`index.css` 的 `.quickforge-chat-panel-host.quickforge-chat-compact` 段将 agent-access/模型/Plan/OpenCode 配置/OpenCode 模式五个带文字控件收缩为 2rem 只留 icon（与移动端 icon-only 形态一致，Side Chat 复用同一宿主同样生效）；移动端窄视口仍走原 `@media (max-width: 768px)` 块
