...
| 文件 | 行数 | 端点 | 用途 |
|------|------|------|------|
| `agent.mjs` | 333 | `/api/agents/*` | Agent 会话管理、消息流式处理 |
| `storage.mjs` | 152 | `/api/storage/*` | 存储 CRUD 操作 |
| `project.mjs` | 107 | `/api/project/*` | 项目管理 |
| `filesystem.mjs` | 88 | `/api/filesystem/*` | 文件系统浏览 |
| `tools.mjs` | 83 | `/api/tools`, `/api/:scope/tools/*` | 工具定义和执行 |
| `skills.mjs` | 146 | `/api/skills` | Skills 管理 |
| `scheduled-tasks.mjs` | 803 | `/api/scheduled-tasks/*` | 定时任务管理 |
| `shares.mjs` | 91 | `/api/shares/*` | 分享管理 |
| `shared-conversation.mjs` | 405 | `/api/shared/*` | 共享会话查看 |
| `backup.mjs` | 251 | `/api/backup/*` | 数据备份和恢复 |
| `instructions.mjs` | 21 | `/api/instructions` | 系统提示词 |
| `system.mjs` | 36 | `/api/system/*` | 系统状态和重启 |
| `static.mjs` | 59 | 静态文件 | Vite 构建产物服务 |
...
**调度引擎**: 内置调度器（`startScheduledTaskRunner`），支持 Cron 表达式和间隔调度，处理任务依赖和并发控制。
