...
describe('workspace inspector width range constants', () => {
  ...
  it('derives the max width from the viewport with an absolute cap and a min-width floor', () => {
    expect(source).toContain('function getInspectorMaxWidth(leftSidebarWidth = 0, conversationMinWidth = 0)')
    expect(source).toContain('window.innerWidth - leftSidebarWidth - conversationMinWidth - 1')
    expect(source).toContain('window.innerWidth * WORKSPACE_INSPECTOR_MAX_VIEWPORT_RATIO')
  })

  it('clamps through clampInspectorWidth in both persisted restore and drag resize', () => {
    expect(source).toMatch(/readPersistedInspectorWidth[\s\S]*?return clampInspectorWidth\(value, leftSidebarWidth, conversationMinWidth\)/)
    expect(source).toMatch(/start\.currentWidth = clampInspectorWidth\(start\.startWidth \+ start\.startX - event\.clientX, leftSidebarWidth, conversationMinWidth\)/)
  })

  it('auto-expands to the fixed AUTO_EXPAND_WIDTH instead of the viewport max', () => {
    expect(source).toMatch(/expandInspectorToMax[\s\S]*?WORKSPACE_INSPECTOR_AUTO_EXPAND_WIDTH[\s\S]*?leftSidebarWidth, conversationMinWidth[\s\S]*?: current/)
