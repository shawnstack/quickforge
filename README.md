...
| 自定义命令 | 支持 `/plan`、`/review`、`/compact`、`/clear` 等内置命令，也可从项目 `.ai/commands/` 或配置目录加载团队命令。`/plan <任务>` 只生成执行计划，本轮禁止写文件、改文件和运行命令；`/review [范围]` 用于提交前自检待提交代码，本轮禁止编辑文件。 |
...
| Custom commands | Includes built-in commands such as `/plan`, `/review`, `/compact`, and `/clear`, and can load project commands from `.ai/commands/` or configured directories. `/plan <task>` drafts an implementation plan without edits or commands; `/review [scope]` performs a pre-commit self-review without editing files. |
