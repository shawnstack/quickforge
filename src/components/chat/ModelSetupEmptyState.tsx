...
export function ModelSetupEmptyState({ onAddModel, onUseExample }: ModelSetupEmptyStateProps) {
  return (
    <div className="flex min-h-0 flex-1 items-center justify-center p-6">
      <div className="w-full max-w-xl rounded-2xl border border-border bg-card/80 p-6 text-center shadow-sm backdrop-blur">
        <div className="mx-auto flex size-12 items-center justify-center rounded-2xl bg-primary/10 text-primary">
          <Bot className="size-6" />
        </div>
        <h2 className="mt-4 text-lg font-semibold text-foreground">{t('modelSetupTitle')}</h2>
        <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-muted-foreground">{t('modelSetupDescription')}</p>

        <div className="mt-5 grid gap-3 rounded-xl border border-border/70 bg-muted/25 p-4 text-left text-sm">
